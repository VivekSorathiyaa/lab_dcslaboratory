...
<section class="content" style="padding: 0px;
     ...
     padding-right: 0px; ">
			...
			<div class="col-md-4">
			<h1 style="">
			<a  class="btn btn-info" href="add_cash_receipt.php" title="ADD RECEIPT" style="margin-left:10px;">Add Cash Payment In</a>
			</h1>
			</div>
			<div class="col-md-3">
			<h1 style="text-align:center;">Cash Payment In</h1>
			</div>
			</div>
...
function search_data(type,id){
    ...
    if (type == 'search_data') {
				...
				billData = '&action_type='+type+'&user_type='+user_type+'&sends='+sends+'&from_date='+from_date+'&to_date='+to_date;		
    }
	
    $.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>search_cash_receipt.php',
        data: billData,
		beforeSend: function(){
...
$(document).on("click",".delete_cash_receipt",function(){
					...
					if(confirm("Are You Sure To Delete..?"))
					{
						...
						var postData = '&action_type=delete_cash_receipt&delete_rec_id='+delete_rec_id;
				
						$.ajax({
							url : "save_cash_receipt.php",
							type: "POST",
							data : postData,
							success: function(data,status,  xhr)
							 {
								alert("Cash Receipt Is Successfully Deleted");
								window.location.href="cash_receipt_list.php";
