...
if($_SESSION['name']=="")
{
	...
	<?php
		...
		if($agency != 0 && $from_date!= "" && $to_date!= "" && $clientname!=0){
			$where="where `date` >= '$new_from_date' AND `date` <='$new_to_date' AND `agency`='$agency' AND `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else if($from_date !== "" && $to_date !== "" && $agency != 0){
			$where="where `date` >= '$new_from_date' AND `date` <='$new_to_date' AND `agency`='$agency' and send_to_second_reception ='1'";
		}
		else if($from_date!= "" && $to_date!= "" && $clientname!=0){
			$where="where `date` >= '$new_from_date' AND `date` <='$new_to_date' AND `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else if($agency != 0  && $to_date!= "" && $clientname!=0){
			$where="where  `date` <='$new_to_date' AND `agency`='$agency' AND `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else if($agency != 0 && $to_date!= "" && $clientname!=0){
			$where=" `date` >= '$new_from_date'  AND `agency`='$agency' AND `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else if($from_date !== "" && $to_date !== "" ){
			$where="where `date` >= '$new_from_date' AND `date` <='$new_to_date' and send_to_second_reception ='1'";
		}
		else if($from_date !== "" && $to_date !== "" && $agency != 0){
			$where="where `date` >= '$new_from_date'  AND `agency`='$agency' and send_to_second_reception ='1'";
		}
		else if($from_date!= "" && $clientname!=0){
			$where="where `date` >= '$new_from_date' AND `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else if($to_date !== "" && $agency != 0){
			$where="where `date` <='$new_to_date' AND `agency`='$agency' and send_to_second_reception ='1'";
		}
		else if($to_date!= "" && $clientname!=0){
			$where="where `date` <='$new_to_date' AND `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else if($agency != 0 && $clientname!=0){
			$where="where `agency`='$agency' AND `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else if($from_date !== "" ){
			$where="where `date` >= '$new_from_date' and send_to_second_reception ='1'";
		}
		else if($to_date !== "" ){
			$where="where `date` <='$new_to_date' and send_to_second_reception ='1'";
		}
		else if($agency != 0){
			$where="where `agency`='$agency'  and send_to_second_reception ='1'";
		}
		else if($clientname!=0){
			$where="where `client_code`='$clientname' and send_to_second_reception ='1'";
		}
		else{
			$where="where send_to_second_reception ='1'";
		}
					...
					<table id="example1" class="table table-bordered table-striped">
								...
								<tbody>
									<?php
										...
										$count=0;
										echo $query="select * from job ".$where;
									...
									?>
											...
											<td style="white-space:nowrap;">
												...
												?>
												<a href="javascript:void(0);" class="btn btn-danger btn-lg btn3d send_to_second" data-id="<?php echo $row['report_no'];?>"title="SEND QUERIES TO RECEPTION"><span class="glyphicon glyphicon-question-ok"></span>EDIT FOR RECEPTION 1</a>
												<?php
												}
												?>
											</td>
