...
if(isset($_POST['action_type']) && !empty($_POST['action_type']))
{
		...
		else if($_POST['action_type'] == 'add_purchase')
		{
			...
			$discripts=$_POST['discripts'];
				...
				$sel_voch="select * from purchages ORDER BY purchage_id DESC LIMIT 0,1";
				$query_vouch=mysqli_query($conn,$sel_voch);
				if(mysqli_num_rows($query_vouch) > 0)
				{
					$one_vouch=mysqli_fetch_array($query_vouch);
					$explodings=explode("/",$one_vouch["purchage_code"]);
					$lasts=intval($explodings[2])+1;
					$sets= sprintf('%04d', $lasts);
					
					$txt_vouch=$pur_in_first_parts.$sets;
					$img_name=$pur_in_img_first_parts.$sets;
				}else{
					$txt_vouch=$pur_in_first_parts."0001";
					$img_name=$pur_in_img_first_parts."0001";
				}
				...
				$todays=date("Y-m-d");
				$insert_job_only="INSERT INTO `purchages`(`purchage_code`,`party_id`, `party_name`, `gst_no`, `bill_no`, `bill_date`, `taxable_amnt`,`sgst`,`cgst`,`igst`,`total_amnt`,`remark`, `created_date`, `bill_upload`, `created_by`) values(
						'$txt_vouch',
						'$party_id',
						'$party_name',
						'$gst_no',
				...
				if(mysqli_num_rows($query_balance) > 0)
				{
					...
					$set_total= floatval($totals) + floatval($total_amnt);
					$set_remain= floatval($remain) + floatval($total_amnt);
					
					
					$up_balance="update party_balance_sheet set `total_balance`='$set_total',`paid_balance`='$paid',`remain_balance`='$set_remain',`last_action`='delete_bill' where `user_type`='party' AND `user_id`='$party_id'";
					mysqli_query($conn,$up_balance);
					
					$ins_ledger="insert into party_ledger(`user_type`,`entry_type`,`user_id`,`user_name`,`bill_no`,`receipt_no`,`entry_date`,`debits`,`credits`,`created_by`,`created_name`)VALUES('party','purchase_in','$party_id','$party_name','$txt_vouch','0','$bill_date','0','$total_amnt','29','KINJAL SORTHIYA')";
		...
		else if($_POST['action_type'] == 'edit_receipt')
		{
			$pur_code=$_POST['pur_code'];
			...
			{
				$ext = pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
				$explodes=explode("/",$_POST['pur_code']);
				$set_name=$explodes[0]."_".$explodes[1]."_".$explodes[2];
				
				$filename= $set_name.".".$ext;
				$tmp_name = $_FILES['file']['tmp_name'];
				$path = 'purchage_in_upload/';
				move_uploaded_file($tmp_name,$path.$filename);
				$where=",`bill_upload`='$filename'";
			}else{
				$where="";
				
			}
			
			$sel_perfomas="select * from purchages where `purchage_id`=".$txt_id;
			$result_perfomas=mysqli_query($conn,$sel_perfomas);
			$get_perfomas=mysqli_fetch_array($result_perfomas);
			$total_amting=$get_perfomas["total_amnt"];
			

				$todays=date("Y-m-d");
				$insert_job_only="update `purchages` set `party_id`='$party_id',`party_name`='$party_name', `gst_no`='$gst_no', `bill_no`='$bill_no', `bill_date`='$bill_date', `taxable_amnt`='$taxable_amnt', `sgst`='$sgst_amnt', `cgst`='$cgst_amnt', `igst`='$igst_amnt', `total_amnt`='$total_amnt', `remark`='$discripts'".$where." where `purchage_id`=".$txt_id;
				mysqli_query($conn,$insert_job_only);
				...
				if(mysqli_num_rows($query_balance) > 0)
				{
					...
					$set_total= floatval($totals) - floatval($total_amting);
					$set_remain= floatval($remain) - floatval($total_amting);
					
					
					$up_balance="update party_balance_sheet set `total_balance`='$set_total',`paid_balance`='$paid',`remain_balance`='$set_remain',`last_action`='update_purchage' where `user_type`='party' AND `user_id`='$party_id'";
					mysqli_query($conn,$up_balance);
					
					$del_legders="delete from party_ledger where `bill_no`='$pur_code'";
					$query_ledgs=mysqli_query($conn,$del_legders);
					
					$ins_ledger="insert into party_ledger(`user_type`,`entry_type`,`user_id`,`user_name`,`bill_no`,`receipt_no`,`entry_date`,`debits`,`credits`,`created_by`,`created_name`)VALUES('party','purchase_in','$party_id','$party_name','$pur_code','0','$bill_date','0','$total_amnt','29','KINJAL SORTHIYA')";
					mysqli_query($conn,$ins_ledger);
				}
				...
				if(mysqli_num_rows($query_balances) > 0)
				{
					...
					$up_balance="update party_balance_sheet set `total_balance`='$set_total',`paid_balance`='$paid',`remain_balance`='$set_remain',`last_action`='update_purchase' where `user_type`='party' AND `user_id`='$party_id'";
					mysqli_query($conn,$up_balance);
		...
		else if($_POST['action_type'] == 'change_amnt')
		{
			...
			echo json_encode($fill);
		}else if($_POST['action_type'] == 'change_gst_slab')
		{
			$taxable_amnt=$_POST['taxable_amnt'];
			$gst_slab=$_POST['gst_slab'];
			$csi=$_POST['csi'];
			$part_gst= floatval($gst_slab) / 2;
			if($csi=="cs")
			{
				$c_gst_amt= floatval($taxable_amnt) / 100 * floatval($part_gst);
				$s_gst_amt= floatval($taxable_amnt) / 100 * floatval($part_gst);
				$i_gst_amt= 0;
			}
			else
			{
				$c_gst_amt= 0;
				$s_gst_amt= 0;
				$i_gst_amt= floatval($taxable_amnt) / 100 * floatval($gst_slab);
			}
			
			$totals= floatval($taxable_amnt) + floatval($c_gst_amt) + floatval($s_gst_amt) + floatval($i_gst_amt);
			
			$fill = array("status" => "1","c_gst_amt" => $c_gst_amt,"s_gst_amt" => $s_gst_amt,"i_gst_amt" => $i_gst_amt,"totals" => $totals);
			echo json_encode($fill);
		}else if($_POST['action_type'] == 'change_party')
		{
			$partys=explode("|",$_POST['party_name']);
			$party_id=$partys[0];
			
			$sel_party="select * from party_master where `party_id`='$party_id'";
			$query_party=mysqli_query($conn,$sel_party);
			$one_party=mysqli_fetch_array($query_party);
			
			$fill = array("party_gst" => $one_party["party_gst"]);
			echo json_encode($fill);
		}else if($_POST['action_type'] == 'delete_vouch')
		{
			$delete_vouch_id=$_POST["delete_vouch_id"];
			$sel_estiamte="select * from purchages where `purchage_id`=".$delete_vouch_id;
			...
			$bill_no=$get_estimate["purchage_code"];
		...
		if(mysqli_num_rows($query_balance) > 0)
		{
			...
			$up_balance="update party_balance_sheet set `total_balance`='$set_total',`paid_balance`='$paid',`remain_balance`='$set_remain',`last_action`='delete_purchase_in' where `user_type`='party' AND `user_id`='$user_id'";
			mysqli_query($conn,$up_balance);
			
			$del_legders="delete from party_ledger where `bill_no`='$bill_no' AND `user_id`='$user_id' AND `entry_type`='purchase_in'";
			$query_ledgs=mysqli_query($conn,$del_legders);
			
		}
			
			
			$del_vouch="delete from purchages where `purchage_id`=".$delete_vouch_id;
