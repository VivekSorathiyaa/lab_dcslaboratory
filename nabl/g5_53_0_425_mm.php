			...
			<div class="col-md-12">
				<div class="box box-info">
					<div class="box-header with-border">
						<h2 style="text-align:center;">GSB - V MIX (M5)</h2>
					</div>
					...
					<div class="panel panel-default" id="grd">
								...
								<div class="row">
									...
									<div class="col-lg-1">
										<div class="form-group">
											<div class="col-sm-12">
												<label for="inputEmail3" class="col-sm-2 control-label">75</label>
											</div>
										</div>
									</div>
									<div class="col-lg-2">
								...
								<div class="row">
									...
									<div class="col-lg-1">
										<div class="form-group">
											<div class="col-sm-12">
												<label for="inputEmail3" class="col-sm-2 control-label">0.425</label>
											</div>
										</div>
									</div>
									<div class="col-lg-2">
									...
									<div class="col-lg-12">
										<div class="form-group">
											...
											<div class="col-sm-1">
												...
												<?php   
													$querys_job1 = "SELECT * FROM gsb_mix_5_m5 WHERE `is_deleted`='0' and lab_no='$lab_no'";
											...
											?>
											<div class="col-sm-2">
												<a target = '_blank' href="<?php echo $base_url; ?>print_report/print_gsb_1_mix_m5.php?job_no=<?php echo $_GET['job_no'];?>&&report_no=<?php echo $_GET['report_no'];?>&&lab_no=<?php echo $_GET['lab_no'];?>&&trf_no=<?php echo $_GET['trf_no'];?>&&ulr=<?php echo $_GET['ulr'];?>&&tbl_name=gsb_mix_5_m5" class="btn btn-info pull-right" id="btn_report" name="btn_report"><b>Report</b></a>
 
											</div>
																					<!--	<div class="col-sm-2">
 													<a target='_blank' href="<?php echo $base_url; ?>back_cal_report_blank/print_gsb_1_mix_m5.php?job_no=<?php echo $_GET['job_no']; ?>&&report_no=<?php echo $_GET['report_no']; ?>&&lab_no=<?php echo $_GET['lab_no']; ?>&&trf_no=<?php echo $_GET['trf_no']; ?>&&ulr=<?php echo $_GET['ulr']; ?>&&tbl_name=gsb_mix_4_m5" class="btn btn-info pull-right" id="btn_report" name="btn_report"><b>Calculation Blank</b></a>

 												</div> -->
											
											<?php //} ?>
											<div class="col-sm-2">
												<a target = '_blank' href="<?php echo $base_url; ?>back_cal_report/print_gsb_1_mix_m5.php?job_no=<?php echo $_GET['job_no'];?>&&report_no=<?php echo $_GET['report_no'];?>&&lab_no=<?php echo $_GET['lab_no'];?>&&trf_no=<?php echo $_GET['trf_no'];?>&&tbl_name=gsb_mix_5_m5" class="btn btn-info pull-right" id="btn_cal_report" name="btn_cal_report"><b>Calculation Report</b></a>
					...
					<div class="col-lg-12">
						<table border="1px solid black" align="center" width="100%" id="aaaa">
							<tr>
								...
								<?php
							 $query = "select * from gsb_mix_5_m5 WHERE lab_no='$aa'  and `is_deleted`='0'";
...
//SOUNDNESS	
	...
	function general_flk_elo1()
	{
		...
		var dd1 = randomNumberFromRange(1000.0,1200.0).toFixed(2);
		var dd2 = randomNumberFromRange(1000.0,1200.0).toFixed(2);			
		var dd3 = randomNumberFromRange(1000.0,1200.0).toFixed(2);			
		...
		var fle_1 = randomNumberFromRange(100.0,100.0);
		var fle_2 = randomNumberFromRange(100.0,100.0);
		var fle_3 = randomNumberFromRange(100.0,100.0);
		...
		var fle_10 = randomNumberFromRange(100.0,100.0);
		var fle_11 = randomNumberFromRange(100.0,100.0);
		var fle_12 = randomNumberFromRange(100.0,100.0);
    ...
    let sieve_sizes = [75, 53, 26.5, 9.5, 4.75, 2.36, 0.425, 0.075];
    let pass_ranges = ["100 %", "80-100 %", "55-90 %", "35-65 %", "25-55 %", "20-40 %", "10-15 %", "0-5 %"];
    let sample_taken = 5000;

    // Set pass ranges
    for (let i = 1; i <= 8; i++) {
        $('#pass_range_' + i).val(pass_ranges[i - 1]);
    }

    // Generate passing sample values within given ranges
    let pass_sample = [
        randomNumberFromRange(100, 100),
        randomNumberFromRange(91.00, 99.00),
        randomNumberFromRange(60.00, 90.00),
        randomNumberFromRange(36.00, 59.00),
        randomNumberFromRange(26.00, 35.00),
        randomNumberFromRange(20.00, 25.00),
        randomNumberFromRange(11.00, 14.00),
        randomNumberFromRange(1.00, 4.00)
       
    ];

    for (let i = 1; i <= 8; i++) {
        $('#pass_sample_' + i).val(pass_sample[i - 1].toFixed());
    }

    // Calculate cumulative retained percentage (100 - passing sample)
    let cum_ret = pass_sample.map(val => (100 - val).toFixed(1));
    for (let i = 1; i <= 8; i++) {
        $('#cum_ret_' + i).val(cum_ret[i - 1]);
    }

    // Compute retained weight (CUMRET * SAMPLE_TAKEN / 100)
    let ret_wt_gm = cum_ret.map(val => ((parseFloat(val) * sample_taken) / 100).toFixed(0));
    for (let i = 1; i <= 8; i++) {
        $('#ret_wt_gm_' + i).val(ret_wt_gm[i - 1]);
    }

    // Compute cumulative weight
    let cum_wt_gm = [];
    cum_wt_gm[0] = ret_wt_gm[0];
    for (let i = 1; i < 8; i++) {
        cum_wt_gm[i] = (parseFloat(ret_wt_gm[i]) - parseFloat(ret_wt_gm[i - 1])).toFixed(0);
    }
    for (let i = 1; i <= 8; i++) {
        $('#cum_wt_gm_' + i).val(cum_wt_gm[i - 1]);
    }

    // Compute sum of cumulative weight
    let blank_extra = cum_wt_gm.reduce((acc, val) => acc + parseFloat(val), 0).toFixed(0);
    $('#blank_extra').val(blank_extra);
	...
	$('#chk_grd').change(function(){
		...
		else
		{
			...
			$('#pass_sample_12').val(null);
			
			$('#pass_range_1').val(null);
			$('#pass_range_2').val(null);
			$('#pass_range_3').val(null);
			$('#pass_range_4').val(null);
			$('#pass_range_5').val(null);
			$('#pass_range_6').val(null);
			$('#pass_range_7').val(null);
			$('#pass_range_8').val(null);
			$('#pass_range_9').val(null);
			$('#pass_range_10').val(null);
			$('#pass_range_11').val(null);
			$('#pass_range_12').val(null);
...
{
	$('#txtgrd').css("background-color","var(--success)");
	var sieve = ["75", "53", "26.5", "9.5", "4.75", "2.36", "0.425", "0.075"];
	
	var pass_ranges = ["100 %", "80-100 %", "55-90 %", "35-65 %", "25-55 %", "20-40 %", "10-15 %", "0-5 %"];
	...
	for (var i = 1; i <= 8; i++) {
		var pass_sample = $('#pass_sample_' + i).val();
		var cum_ret = 100 - (+pass_sample);
		$('#cum_ret_' + i).val(cum_ret.toFixed(1));
	}

	var cum_ret = [];
	for (var i = 1; i <= 8; i++) {
		cum_ret[i] = $('#cum_ret_' + i).val();
	}

	var ret_wt_gm = [];
	for (var i = 1; i <= 8; i++) {
		ret_wt_gm[i] = (parseFloat(cum_ret[i]) * parseFloat(sample_taken)) / 100;
		$('#ret_wt_gm_' + i).val(ret_wt_gm[i].toFixed(0));
	}

	var cum_wt_gm = [];
	cum_wt_gm[1] = ret_wt_gm[1];
	for (var i = 2; i <= 8; i++) {
		cum_wt_gm[i] = parseFloat(ret_wt_gm[i]) - parseFloat(ret_wt_gm[i - 1]);
		$('#cum_wt_gm_' + i).val(cum_wt_gm[i].toFixed(0));
	}
	
	var blank_extra = 0;
	for (var i = 1; i <= 8; i++) {
		blank_extra += +cum_wt_gm[i];
	}
	$('#blank_extra').val(blank_extra.toFixed(0));
}

	
	$('#sample_taken,#pass_sample_1,#pass_sample_2,#pass_sample_3,#pass_sample_4,#pass_sample_5,#pass_sample_6,#pass_sample_7,#pass_sample_8,#pass_sample_9,#pass_sample_10,#pass_sample_11,#pass_sample_12').change(function(){			
		$('#txtgrd').css("background-color","var(--success)");
    ...
    for (let i = 1; i <= 8; i++) {
        cum_wt_gm[i] = $('#cum_wt_gm_' + i).val();
    }

    // MINUS PLUS
    ret_wt_gm[1] = cum_wt_gm[1];
    for (let i = 2; i <= 8; i++) {
        ret_wt_gm[i] = (+cum_wt_gm[i]) + (+ret_wt_gm[i - 1]);
    }

    for (let i = 1; i <= 8; i++) {
        $('#ret_wt_gm_' + i).val(ret_wt_gm[i]);
    }

    var blank_extra = cum_wt_gm.reduce((a, b) => +a + +b, 0);
    $('#blank_extra').val(blank_extra.toFixed(0));

    for (let i = 1; i <= 8; i++) {
        cum_ret[i] = ((+ret_wt_gm[i]) / (+sample_taken)) * 100;
        $('#cum_ret_' + i).val(cum_ret[i].toFixed(1));
    }

    pass_sample[1] = 100;
    for (let i = 1; i <= 8; i++) {
        pass_sample[i] = (+100) - (+cum_ret[i]);
        $('#pass_sample_' + i).val(pass_sample[i].toFixed());
    }
	...
	$('#cum_wt_gm_1,#cum_wt_gm_2,#cum_wt_gm_3,#cum_wt_gm_4,#cum_wt_gm_5,#cum_wt_gm_6,#cum_wt_gm_7,#cum_wt_gm_8,#cum_wt_gm_9,#cum_wt_gm_10,#cum_wt_gm_11,#cum_wt_gm_12').change(function(){
	  weight_cum_gm();
	});
...
function getGlazedTiles(){
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo $base_url; ?>saveg5_53_0_425_mm.php',
