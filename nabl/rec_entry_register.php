			...
			<div class="nav-tabs-custom">
				<div class="tab-content">
					...
					<form class="" method="post" >
					   <div class="row">
									<div class="col-md-12">
										<select class="form-control select2 col-sm-12" tabindex="6"  data-placeholder="Select Material" id="sel_user" name="sel_user">
											<option value="">Select User</option>
											<?php
											$select_query = "select * from multi_login WHERE `staff_isdeleted`='0'";
											$result_select = mysqli_query($conn,$select_query);
											if(mysqli_num_rows($result_select)>0)
											{
											    while($row_users =mysqli_fetch_array($result_select))
												{ ?>
													<option value="<?php echo $row_users['id'];?>"><?php echo $row_users['staff_fullname'];?></option>
												<?php }
											}
											?>
...
function search_agency(type,id){
    ...
    if (type == 'search') {
				...
				billData = '&action_type='+type+'&sel_user='+sel_user+'&todate='+todate+'&fromdate='+fromdate;
    ...
    $.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>search_rec_entry_register.php',
        data: billData,
		beforeSend: function(){
