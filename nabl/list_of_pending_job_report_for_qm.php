					...
					<div class="box-body">
						<div id="display_data_report">
								...
								<tbody>
									<?php
										$count=0;
										//$query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `morr`= 'r' AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `report_job_printing`='1' AND `appoved_by_qm_to_print`=0 AND `job_owner_eng_and_qm`=2 ORDER BY job_id DESC";
										
										$query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `report_job_printing`='1'  AND `job_owner_eng_and_qm`=1  ORDER BY job_id DESC";
									...
									?>
											...
											//$final_query_off="select * from job_for_engineer WHERE `isdeleted`=0 AND `job_no`='$row[job_number]' AND `report_no`='$row[report_no]' AND `eng_light_status`=0";
											//$final_result_off=mysqli_query($conn,$final_query_off);
											//
											//$final_query_working="select * from job_for_engineer WHERE `isdeleted`=0 AND `job_no`='$row[job_number]' AND `report_no`='$row[report_no]' AND `eng_light_status`=1";
											//$final_result_working=mysqli_query($conn,$final_query_working);
											//
											//$final_query_done="select * from job_for_engineer WHERE `isdeleted`=0 AND `job_no`='$row[job_number]' AND `report_no`='$row[report_no]' AND `eng_light_status`=2";
										...
										?>
											...
											<a href="javascript:void(0);" class="btn btn-success btn-lg btn3d job_send_to_accept" data-id="<?php echo $row['trf_no']."|".$row['job_number'];?>"title="Material Assign"><span class="glyphicon glyphicon-question-list"></span> Accept job</a>
