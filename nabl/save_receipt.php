...
if(isset($_POST['action_type']) && !empty($_POST['action_type']))
{
		...
		else if($_POST['action_type'] == 'add_receipt')
		{
			...
			$balance="select * from st_balance_sheet where `user_id`='$user_ids' AND `user_type`='$user_type'";
				...
				if(mysqli_num_rows($query_balance) > 0)
				{
					...
					$up_balance="update st_balance_sheet set `paid_balance`='$set_paid',`remain_balance`='$set_remain',`last_action`='Receipt_add' where `user_type`='$user_type' AND `user_id`='$user_ids'";
					mysqli_query($conn,$up_balance);
				}else{
					$names="";
				}
				
				    $sel_voch="select * from st_receipt ORDER BY receipt_id DESC LIMIT 0,1";
					$query_vouch=mysqli_query($conn,$sel_voch);
					if(mysqli_num_rows($query_vouch) > 0)
					{
						$one_vouch=mysqli_fetch_array($query_vouch);
						...
						$txt_vouch=$reciept_first_parts.$sets;
					}else{
						$txt_vouch=$reciept_first_parts."0001";
					}
				$todays=date("Y-m-d");
				$insert_job_only="INSERT INTO `st_receipt`(`user_type`, `user_id`, `user_name`, `receipt_code`, `receipt_date`, `payment_amount`, `tds_amout`, `total_amount`, `payment_type`, `remark`, `created_by`, `created_date`) values(
						'$user_type',
						'$user_ids',
						'$names',
						'$txt_vouch',
				...
				$ins_ledger="insert into st_ledger(`user_type`,`entry_type`,`user_id`,`user_name`,`bill_no`,`receipt_no`,`entry_date`,`debits`,`credits`,`created_by`,`created_name`)VALUES('$user_type','receipting','$user_ids','$names','0','$txt_vouch','$receipt_date','$total_amounts','0','29','KINJAL SORTHIYA')";
		...
		else if($_POST['action_type'] == 'delete_receipt')
		{
			$delete_rec_id=$_POST["delete_rec_id"];
			
			$sel_voch="select * from st_receipt where `receipt_id`=".$delete_rec_id;
			$query_vouch=mysqli_query($conn,$sel_voch);
			if(mysqli_num_rows($query_vouch) > 0)
			{
				$one_vouch=mysqli_fetch_array($query_vouch);
				...
				$sel_balances="select * from st_balance_sheet where `user_type`='$user_type' AND `user_id`='$user_id'";
				...
				if(mysqli_num_rows($query_balances) > 0)
				{
					...
					$up_balance="update st_balance_sheet set `paid_balance`='$set_paid',`remain_balance`='$set_remain',`last_action`='receipt_delete' where `user_type`='$user_type' AND `user_id`='$user_id'";
					mysqli_query($conn,$up_balance);
			...
			$del_vouch="delete from st_receipt where `receipt_id`=".$delete_rec_id;
