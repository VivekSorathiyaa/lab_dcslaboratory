...
$select_test_wise="select * from test_wise_material_rate where `trf_no`='$txt_trf_no' AND `job_no`='$txt_jobs'";
 ...
 $sel_estiamte="select * from estimate_total_span_only_for_test where `trf_no`='$txt_trf_no' AND `job_no`='$txt_jobs'";
					...
					<div class="box box-info">
							...
							<input type="hidden" name="hidden_agency" id="hidden_agency" value="<?php echo $get_agency_id;?>">
								<div class="row">
									...
									<a href="matt_invoice_bill_by_test_print.php?trf_no=<?php echo $get_estimate['trf_no'];?>&&job_no=<?php echo $get_estimate['job_no'];?>" class="btn btn-info" title="" target="_blank" style="width:150px;font-size:20px;"><span class="glyphicon glyphicon-question-list"></span> Print</a>
...
$("#select_rate").change(function(){
      ...
      var postData = 'action_type=get_data_by_rate&select_rate='+select_rate+'&test_id_array='+test_id_array+'&test_qty_array='+test_qty_array;
		  ...
		  return false;
			...
			$.ajax({
				url : "<?php $base_url; ?>save_span_set_rate_for_test.php",
				type: "POST",
				data : postData,
				dataType:'JSON',
		...
		if(hidden_gst_type !="without_gst")
		{
			...
			else
			{
				...
				success:function(msg){
					...
					window.location.href="<?php echo $base_url; ?>span_set_rate_only_by_test.php?trf_no="+txt_trf_no+"&&job_no="+txt_job_no;
