...
<div class="row">
		...
		<h1 style="text-align:center;">
		View Undispatched Jobs For Reception
		</h1>
	</div>		
	<div class="row m-0">
        <div class="col-xs-12">
						...
						<div id="display_data_report">
								...
								<tbody>
									<?php
										...
										$query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `report_job_printing`='1' AND `any_report_done_by_any_qm`='1' AND `dispatch_by_reception`='0' AND `jobcreatedby_id`='$_SESSION[u_id]'  ORDER BY job_id DESC LIMIT 0,100";
										...
										while($row=mysqli_fetch_array($result))
										{
											...
											$sel_completed_report_by_trf="select `dispatch_by_reception` from final_material_assign_master WHERE `dispatch_by_reception`='1' AND `trf_no`='$row[trf_no]'";
											$result_completed_report_by_trf=mysqli_query($conn,$sel_completed_report_by_trf);
											if(mysqli_num_rows($result_completed_report_by_trf) > 0)
											{ $set_colors="btn-warning";}else{ $set_colors="btn-primary"; }
									...
									?>
											...
											<?php if($row["morr"]=="r")
											{ ?>
											<a href="view_job_by_reception.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>" class="btn <?php echo $set_colors;?>" title="View Job"><span class="fa fa-truck"></span></a>
...
$(".search_job_by_rec").click(function()
{
	...
	if(search_trf_no =="" && search_sam_date =="" && search_sam_month =="")
	{
		alert("Please Atlist One select");
		return false;
	}
		var postData = '&type=search_all_job_for_admin&search_trf_no='+search_trf_no+'&search_sam_date='+search_sam_date+'&search_sam_month='+search_sam_month;
			
		$.ajax({
			url : "<?php echo $base_url; ?>search_un_dispatched_reports_for_rec.php",
			type: "POST",
			data : postData,
			beforeSend: function(){
