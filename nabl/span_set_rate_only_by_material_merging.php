...
$sel_estiamte = "select * from estimate_total_span_only_for_material where `trf_no`='$chk_array' AND `job_no`='$chk_array'  AND `est_isdeleted`=0";
...
if (mysqli_num_rows($estiamte_query) > 0) {
	...
	$sel_perfoma = "select * from estimate_total_span where `trf_no`='$_GET[chk_array]' AND `job_no`='$_GET[chk_array]'  AND `est_isdeleted`=0";
	...
	foreach ($explode_chk_array as $keys => $one_chk_arrays) {
		$sel_test_mater = "select *,`final_material_id`,count(final_material_id) as get_count from test_wise_material_rate where `trf_no`='$one_chk_arrays' AND `job_no`='$one_chk_arrays'  AND `is_deleted`=0 GROUP BY final_material_id";
...
foreach ($explode_chk_array as $one_chk_array) {
						...
						<div class="box box-info">
							...
							<div class="row">
								...
								<div class="col-md-3">
									...
									<a href="matt_invoice_bill_by_material_print.php?chk_array=<?php echo $chk_array; ?>" class="btn btn-info" title="" target="_blank" style="width:150px;font-size:20px;"><span class="glyphicon glyphicon-question-list"></span> Print</a>
									<br>
								</div>
								<div class="col-md-3">&nbsp;</div>
...
<script>
	...
	function get_table_after_update(id) {
		...
		$.ajax({
			type: 'POST',
			url: '<?php $base_url; ?>save_span_set_rate_material_merging.php',
			data: billData,
			dataType: 'JSON',
	...
	function addData(type, id) {
		...
		$.ajax({
			...
			success: function(msg) {
				...
				window.location.href = "<?php echo $base_url; ?>span_set_rate_only_by_material_merging.php?chk_array=" + txt_trf_no;
