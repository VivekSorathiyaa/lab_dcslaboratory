...
if(isset($_POST["save_next"]))
{
   ...
   $select_mate_counts="select `final_material_id`,`material_category` from final_material_assign_master WHERE `is_deleted`=0 AND `temporary_trf_no`='$txt_final_trf_no' AND `created_by_id`='$_SESSION[u_id]' AND `report_no`='0' ORDER BY final_material_id ASC";
	...
	$get_counts_of_materials=mysqli_num_rows($result_mate_counts);
		...
		if($get_counts_of_materials > 0)
		{
			...
			$select_done_final="select * from final_material_assign_master WHERE `is_deleted`=0 AND `temporary_trf_no`='$txt_final_trf_no' AND `created_by_id`='$_SESSION[u_id]' AND `report_no` !='0' ORDER BY final_material_id DESC";
			$result_done_final=mysqli_query($conn,$select_done_final);
			$get_done_final= mysqli_fetch_assoc($result_done_final);
			
			$last_done_final= $get_done_final["report_no"];
			$explode_reports_no= explode(".",$last_done_final);
			$report_no_plusing= intval($explode_reports_no[1])+1;
			
			$limit_for_final =intval($explode_reports_no[1])+ intval($get_counts_of_materials);
			...
			for($i=$report_no_plusing;$i<=$limit_for_final;$i++)
			{
				...
				$update_final_mate="update final_material_assign_master set `trf_no`='$set_trf_no',`job_no`='$set_job_no',`lab_no`='$txt_set_lab_nos',`ulr_no`='$txt_final_ulr_no',`report_no`='$set_report_nos',`is_status`=1 where `final_material_id`=".$final_id_array[$counting];
