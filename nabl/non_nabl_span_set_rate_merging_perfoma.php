...
if($_SESSION['name']=="")
{
	...
	$get_temporary_trf_no= explode(",",$_GET["temporary_trf_no"]);

	$txt_trf_no= $get_chk_array[0];
	$txt_jobs= $get_chk_array[0];
	$temporary_trf_no= $get_temporary_trf_no[0];
...
$sel_estiamte="select * from estimate_total_span where `trf_no`='$_GET[chk_array]' AND `job_no`='$_GET[chk_array]' AND `temporary_trf_no`='$_GET[temporary_trf_no]'  AND `nabl_type`='non_nabl'";
 ...
 if(mysqli_num_rows($estiamte_query) > 0){
	 ...
	 $other_customer_gst_no= "";
	  ...
	  $gst_in_or_ex="";
		...
		foreach($get_chk_array as $keyss => $one_chk_array)
		{
			$sel_jobs_doc="select * from `job` where `trf_no`='$one_chk_array' AND `temporary_trf_no`='$get_temporary_trf_no[$keyss]'";
					...
					<div class="box box-info">
							...
							<br>
								...
								<div class="row">
										  ...
										  <div class="col-md-3">
												<input type="text"  class="form-control" name="hsn_codes" id="hsn_codes" value="998346">
										  </div>
										  <div class="col-md-3">
							...
							<div class="col-md-2">
									...
									if($is_estimate_avail=="not_available")
									{
										...
										$count_rows=0;
										foreach($get_chk_array as $keying => $one_chk_array)
									{

											$select_final_mat="select * from final_material_assign_master where `trf_no`='$one_chk_array' AND `job_no`='$one_chk_array' AND `temporary_trf_no`='$get_temporary_trf_no[$keying]'";

											$get_final_mat=mysqli_query($conn_of_non,$select_final_mat);
											...
											$select_test_wise="select * from test_wise_material_rate where `final_material_id`='$one_final_mat_id[final_material_id]' AND `material_cat_id`='$one_final_mat_id[material_category]' AND `material_id`='$one_final_mat_id[material_id]'";
											 $select_test_wise_query=mysqli_query($conn_of_non,$select_test_wise);
											...
											$get_total_amt=0;
													...
													while($get_test=mysqli_fetch_array($select_test_wise_query))
													{
														...
														$sel_test_query=mysqli_query($conn_of_non,$sel_test_name);
														$get_test_record=mysqli_fetch_array($sel_test_query);

														/* $sel_mate="select * from material where `id`=".$get_test["material_id"];
														$sel_mate_query=mysqli_query($conn,$sel_mate);
														$get_mate=mysqli_fetch_array($sel_mate_query);
														$mate_names=$get_mate["mt_name"];
														$cat_ids=$get_mate["mat_category_id"]; */

														$sel_cats="select * from material_category where `material_cat_id`=".$get_test_record["mat_category_id"];
														$sel_cats_query=mysqli_query($conn_of_non,$sel_cats);
														...
														if (!in_array($get_test_record["test_id"], $merge_test_id_array))
														{
															...
															array_push($merge_test_qty_array,1);
															array_push($merge_test_rate_array,$get_test_record["test_rate_gov"]);
															array_push($merge_test_amt_array,$get_test_record["test_rate_gov"]);
														}
														else
														{
															...
															$plus_qty= intval($get_qty_by_key)+ intval(1);
															...
															$plus_amt= intval($get_amt_by_key)+ intval($get_test_record['test_rate_gov']);
														  ...
														  <input type="text" name="mat_name[]" class="form-control class_mate_name" id="matename_<?php echo $keyer;?>" value="<?php echo $merge_material_name_array[$keyer]; ?>" disabled style="width:200px;font-weight:bold;">
														  </td>
														  <td>
							...
							<input type="hidden" name="hidden_agency" id="hidden_agency" value="<?php echo $get_agency_id;?>">
								<div class="row">
									...
									<a href="non_nabl_matt_perfoma_bill_print.php?chk_array=<?php echo $get_estimate['trf_no'];?>&&temporary_trf_no=<?php echo $get_estimate['temporary_trf_no'];?>" class="btn btn-info" title="" target="_blank" style="width:150px;font-size:20px;"><span class="glyphicon glyphicon-question-list"></span> Print</a>
...
$("#select_rate").change(function(){
      ...
      var postData = 'action_type=get_data_by_rate&select_rate='+select_rate+'&test_id_array='+test_id_array+'&test_qty_array='+test_qty_array;
		  ...
		  return false;
			...
			$.ajax({
				url : "<?php $base_url; ?>non_nabl_save_span_set_rate_merging_perfoma.php",
				type: "POST",
				data : postData,
				dataType:'JSON',
				success: function(data)
		...
		if(hidden_gst_type !="without_gst")
		{
			...
			else
			{
				...
				success:function(msg){
					...
					window.location.href="<?php echo $base_url; ?>non_nabl_span_set_rate_merging_perfoma.php?chk_array="+txt_trf_no+'&&temporary_trf_no='+txt_temporary_trf_no;
