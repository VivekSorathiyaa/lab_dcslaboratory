...
$query_agency= mysqli_query($conn_of_non,$sel_agency);
							...
							<input type="hidden" name="hidden_agency" id="hidden_agency" value="<?php echo $get_agency_id;?>">
								...
								<div class="table-responsive" style="min-height:150px;" border="1">
										<table class="table no-margin">
										  ...
										  <?php
                                          $sel_excel="select * from estimate_total_span where `trf_no`='$txt_trf_no' AND `est_isdeleted`=0 AND `temporary_trf_no`='$temporary_trf_no'";
										  ...
										  ?>
											  <tr>
												...
												<td><?php echo $one_excel["perfoma_no"];?></td>
												<td><?php echo $one_excel["perfoma_excel_upload"];?></td>
												<td><?php echo $one_excel["total_amt"];?></td>
												<td>
												<a class="btn btn-success" href="perfoma_excel/<?php echo $one_excel["perfoma_excel_upload"];?>" target="_blank"style="width:150px;font-size:20px;">Download</a>
												&nbsp;
												<button type="button" class="btn btn-danger delete_perfoma_excel"  id="<?php echo $one_excel["perfoma_no"];?>" style="width:150px;font-size:20px;">Delete</button>
												</td>
											  </tr>
										  <?php
										    }
								...
								<div class="row">
									...
									<!--<a href="matt_perfoma_bill_print.php?chk_array=<?php //echo $get_estimate['trf_no'];?>" class="btn btn-info" title="" target="_blank" style="width:150px;font-size:20px;"><span class="glyphicon glyphicon-question-list"></span> Print</a>-->
...
$(document).on("change","#invoice_date",function(){
      ...
      var postData = 'action_type=set_perfoma_no_by_invoice_date&invoice_date='+invoice_date;
			...
			$.ajax({
				url : "<?php $base_url; ?>non_nabl_save_span_perfoma_excel_upload.php",
				type: "POST",
				data : postData,
				dataType:'JSON',
...
$(document).on("click",".delete_perfoma_excel",function(){

    var perfoma_no=$(this).attr("id");
    var txt_trf_no=$("#txt_trf_no").val();
    var txt_temporary_trf_no=$("#txt_temporary_trf_no").val();
	billData = '&action_type=delete_perfoma_excel&perfoma_no='+perfoma_no;
    ...
    $.ajax({
		...
		dataType:'JSON',
        success:function(msg){
          window.location.href="<?php $base_url; ?>non_nabl_span_perfoma_excel_upload.php?chk_array="+txt_trf_no+"&&temporary_trf_no="+txt_temporary_trf_no;
		}
    });
});
...
$("input[name='gst_type']").change(
    function(e)
    {
		...
		var txt_temporary_trf_no=$("#txt_temporary_trf_no").val();
			...
			total_grand_amnt = parseInt($("#txt_grand").val());

					var txt_job_no=$("#txt_job_no").val();
					var send= "0"+"|"+txt_trf_no+"|"+txt_job_no+"|"+gst_type+"|"+in_or_ex+"|"+total_grand_amnt+"|"+txt_temporary_trf_no;
...
function addData(type,id){
    ...
    if (type == 'add_estimate' || 'save_next_estimate' || 'save_next_estimate_only_save') {
				...
				billData = '&action_type='+type+'&id='+id+'&txt_trf_no='+txt_trf_no+'&txt_temporary_trf_no='+txt_temporary_trf_no;
				...
				success:function(msg){
					...
					window.location.href="<?php echo $base_url; ?>non_nabl_span_perfoma_excel_upload.php?chk_array="+txt_trf_no+"&&temporary_trf_no="+txt_temporary_trf_no;
...
function saveclient(type,id){
    ...
    if (type == 'upload_excel') {
	   ...
	   form_data = new FormData();
		...
		form_data.append("squence_no", squence_no);
    ...
    $.ajax({
        ...
        success:function(data){
		    ...
		    window.location.href="<?php $base_url; ?>non_nabl_span_perfoma_excel_upload.php?chk_array="+txt_trf_no+'&txt_temporary_trf_no='+txt_temporary_trf_no;
