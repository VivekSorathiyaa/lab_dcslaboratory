...
$sel_estiamte = "select * from estimate_total_span_only_for_material where `trf_no`='$chk_array' AND `job_no`='$chk_array'  AND `est_isdeleted`=0 AND `nabl_type`='non_nabl' AND `temporary_trf_no`='$temporary_trf_no_array'";
...
if (mysqli_num_rows($estiamte_query) > 0) {
	...
	$query_perfoma = mysqli_query($conn, $sel_perfoma);
	$result_perfoma = mysqli_fetch_array($query_perfoma);
	$perfoma_nos = $result_perfoma["perfoma_no"];
		...
		if (mysqli_num_rows($perfoma_query) > 0) {
			while ($one_perfoma = mysqli_fetch_array($perfoma_query)) {
				...
				if (!in_array($material_cat_id, $get_material_id_array)) {
					...
					array_push($get_material_qty_array, $one_perfoma["qty"]);
					array_push($get_material_rate_array, $material_rates);
				} else {
					$keys = array_search($material_cat_id, $get_material_id_array);
					$plus = intval($get_material_qty_array[$keys]) + intval($one_perfoma["qty"]);
...
if (mysqli_num_rows($query_estiamte_for_bill_no) > 0) {
	...
	$query_year = mysqli_query($conn_of_non, $sel_year);
...
foreach ($explode_chk_array as $keyes => $one_chk_array) {
	$sel_jobs_doc = "select * from `job` where `trf_no`='$one_chk_array' AND `temporary_trf_no`='$temporary_trf_no_array[$keyes]'";
				...
				<div class="box box-info">
					...
					<div class="box-body" style="border:1px groove #ddd;">
						...
						<div class="row">
							...
							<div class="col-md-3">
								<input type="text" class="form-control" name="bill_no" id="bill_no" value="<?php if ($bill_no == "") {
																												echo $set_bill_no;
																											} else {
																												echo $bill_no;
																											} ?>">
												...
												?>
														<tr>
															<td>
																...
																<input type="text" name="material_hsn_codes[]" class="form-control class_hsn_codes" id="material_hsn_codes<?php echo $mat_key; ?>" value="<?php echo $explode_hsn_codes_array[$mat_key]; ?>">
															</td>
															<td>
													...
													?>
														<tr>
															<td>
																<input type="text" name="material_hsn_codes[]" class="form-control class_hsn_codes" id="material_hsn_codes" value="998346">
															</td>
															<td>
															...
															<td>
																<input type="text" name="material_qty[]" class="form-control class_material_qty" id="material_qty" value="<?php echo $get_material_qty_array[$mat_key]; ?>">
															</td>
															<td>
						...
						<div class="box box-info">
							...
							<div class="row">
								...
								<div class="col-md-3">
									<button type="button" class="btn btn-info" onclick="addData('add_estimate_only_for_material')" name="btn_add_data" id="btn_add_data" style="width:100px;font-size:20px;">Save</button>

									<a href="non_nabl_matt_invoice_bill_by_material_print.php?chk_array=<?php echo $chk_array; ?>&&temporary_trf_no=<?php echo $temporary_trf_no_array ?>" class="btn btn-info" title="" target="_blank" style="width:150px;font-size:20px;"><span class="glyphicon glyphicon-question-list"></span> Print</a>
									<br>
								</div>
								<div class="col-md-3">&nbsp;</div>
...
<script>
	...
	function get_table_after_update(id) {
		...
		$.ajax({
			type: 'POST',
			url: '<?php $base_url; ?>non_nabl_save_span_set_rate_material_merging.php',
			data: billData,
			dataType: 'JSON',
	...
	function addData(type, id) {
		...
		var billData = '';
		if (type == 'add_estimate_only_for_material') {
			var txt_trf_no = $('#txt_trf_no').val();
		...
		$.ajax({
			...
			success: function(msg) {
				...
				window.location.href = "<?php echo $base_url; ?>non_nabl_span_set_rate_only_by_material_merging.php?chk_array=" + txt_trf_no + "&&temporary_trf_no=" + txt_temporary_trf_no;
