...
?>								
									...
									<input type="hidden" name="chk_job_card" value="<?php echo $base_url; ?>set_purchase_print.php?&&start_date=<?php echo $start_date;?>&&end_date=<?php echo $end_date;?>">
									...
									<tr>
										...
										<th style="text-align:center;">Bill Upload</th>
								...
								<tbody>
									<?php
										...
										$query="select * from purchages where `is_deleted`=0".$where." ORDER BY purchage_id ASC";
									...
									?>
											...
											<td><?php echo $row["purchage_code"];?></td>
											<td><?php echo $row["party_name"];?></td>
											<td><?php echo $row["gst_no"];?></td>
											<td><?php echo $row["bill_no"];?></td>
											<td><?php echo date('d/m/Y', strtotime($row['bill_date']));?></td>
											<td><?php echo $row["taxable_amnt"];?></td>
											<td><?php echo $row["cgst"];?></td>
											<td><?php echo $row["sgst"];?></td>
											<td><?php echo $row["igst"];?></td>
											<td><?php echo $row["total_amnt"];?></td>
											<td>
											<?php if($row["bill_upload"]!=""){ ?>
												<a href="purchage_in_upload/<?php echo $row['bill_upload'];?>" target="_blank" class="fa fa-eye"></a>
											<?php } ?>
											</td>
											<td><?php echo $row["remark"];?></td>
											<td>
										<a href="edit_purchage.php?ids=<?php echo $row['purchage_id'];?>" class="btn btn-primary" title="Edit"><i class="fa fa-edit"></i></a>
											
											<a href="javascript:void(0)" class="btn btn-danger delete_vouch" title="Delete" data-id="<?php echo $row['purchage_id'];?>">
