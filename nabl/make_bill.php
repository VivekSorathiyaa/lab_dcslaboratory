					...
					<div class="box box-info">
							...
							<div class="box-body"  style="border:1px groove #ddd;">
								...
								<div class="row">
									...
									<div class="col-md-2">
									<label for="inputEmail3" class="col-md-12 control-label">&nbsp;</label>
									</div>
								</div>
								<div class="row">
										  ...
										  <input type="hidden" id="trf_no" value="<?php echo $get_estimate["trf_no"];?>">
										  </div>
										  <div class="col-md-2"><?php echo $estimate_perfoma_no;?></div>
										  <div class="col-md-2">
											<input type="text"  class="form-control" name="invoice_dates" id="invoice_dates" value="<?php echo date('d/m/Y')?>">
										  </div>
										  <div class="col-md-2">
											<input type="hidden"  class="form-control" name="invoice_number" id="invoice_number" placeholder="If Want To Enter..">
											<input type="hidden" value="<?php echo $make_by;?>" id="make_by">
										  </div>
							...
							<div class="row">
									...
									<button type="button" class="btn btn-info"  onclick="addData('save_final_bill')" name="btn_add_data" id="btn_add_data" style="width:150px;font-size:14px;" >Save</button>
									</div>
...
function addData(type,id){
    ...
    var billData = '';
    if (type == 'save_final_bill') {
				var invoice_dates = $('#invoice_dates').val(); 
				var estimate_perfoma_no = $('#estimate_perfoma_no').val(); 
				var invoice_number = $('#invoice_number').val();
				var est_id = $('#est_id').val();
				var make_by = $('#make_by').val();
				var trf_no = $('#trf_no').val();
				
				billData = '&action_type='+type+'&invoice_dates='+invoice_dates+'&estimate_perfoma_no='+estimate_perfoma_no+'&invoice_number='+invoice_number+'&est_id='+est_id+'&trf_no='+trf_no;
	}
		if(invoice_dates=="")
		{
			...
			$.confirm({
        title: "warning",
        content: "Are You Sure To Save As Invoice ?",
        buttons: {
			confirm: function () {
			$.ajax({
			type: 'POST',
				...
				if(msgs.set_status=="0")
					{
						...
						if(make_by=="0")
						{
						window.location.href="<?php echo $base_url; ?>invoice_print.php?perfoma_nos="+estimate_perfoma_no;
						}else{
							
						window.location.href="<?php echo $base_url; ?>invoice_print_by_material.php?perfoma_nos="+estimate_perfoma_no;
						}
					}
			}
			});
