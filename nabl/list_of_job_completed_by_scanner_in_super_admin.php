						...
						<div id="display_data_report">
								...
								<tbody>
									<?php
										...
										$query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `report_job_printing`='1' AND `any_report_done_by_any_qm`='1' AND `job_completed_by_scanner`='1' ORDER BY job_id DESC";
									...
									?>
											...
											<a href="javascript:void(0);" class="btn btn-warning btn-lg btn3d reward_to_scanner" data-id="<?php echo $row['trf_no'];?>" title="Merge"><span class="glyphicon glyphicon-question-ok"></span>Reward</a>
											</td>
										</tr>
									<?php
										//}	
...
$(document).on("click", ".reward_to_scanner", function () {
	var clicked_id = $(this).attr("data-id");  
				
    
	$.confirm({
        title: "warning",
        content: "Are You Sure To Reward this job To Scanner?",
        buttons: {
			confirm: function () {
	$.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>save_upload_file_for_trf_by_scanner.php',
        data: 'action_type=reward_to_scanner&clicked_id='+clicked_id,
        success:function(html){
			window.location.href="<?php echo $base_url; ?>list_of_job_completed_by_scanner_in_super_admin.php";
