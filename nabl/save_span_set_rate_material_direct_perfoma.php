...
if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
	...
	}else if($_POST['action_type'] == 'add_estimate_only_for_material_direct_perfoma')
	{
		...
		$sel_estimate="select * from estimate_total_span_only_for_material where `perfoma_no`='$hidden_perfoma_nos'";
		$result_estimate=mysqli_query($conn,$sel_estimate);
		
		if(mysqli_num_rows($result_estimate) > 0){
			
		$update_est="update estimate_total_span_only_for_material set `estimate_date`='$invoice_date',`bill_to`='$txt_bill_to',`gst_type`='$hidden_gst_type',`gst_in_or_ex`='$hidden_gst_in_ex',`c_gst_amt`='$txt_cgst',`s_gst_amt`='$txt_cgst',`i_gst_amt`='$txt_igst',`sub_total`='$txt_sub_total',`discount_percent`='$txt_discount_percent',`discount_amount`='$txt_discount_amount',`grand_total`='$txt_grand',`total_amt`='$total_amt',`total_amt_in_word`='$txt_amt_in_word',`hsn_codes`='$all_hsn_codes',`all_material_id`='$all_material_id',`all_material_name`='$all_material_name',`all_material_qty`='$all_material_qty',`all_material_rates`='$all_material_rates',`all_material_amt`='$all_material_amt',`agency_id`='$hidden_agency',`customer_name`='$customer_name',`name_of_work`='$name_of_work',`agreement_no`='$agreement_no',`reference_no`='$reference_no',`other_customer_name`='$other_customer_name',`other_customer_address`='$other_customer_address',`other_customer_gst_no`='$other_customer_gst_no' where `perfoma_no`='$hidden_perfoma_nos'";
		$result_estimate=mysqli_query($conn,$update_est);
		
		
		$up_perfomas="update estimate_total_span set `make_material_bill`='1',`make_estimate`='0',`make_test_bill`='0' where `perfoma_no`='$hidden_perfoma_nos'";
		...
		}else
		{
			...
			$todays=date("Y-m-d");
			$insert_sequence="insert into estimate_total_span_bill_sequence(`trf_no`,`job_no`,`bill_no`,`estimate_type`,`estimate_date`,`created_date`,`created_by`,`created_name`,`perfoma_no`) values('$txt_trf_no','$txt_job_no','$set_bill_no','for_material_direct_perfoma','$invoice_date','$todays','$_SESSION[u_id]','$_SESSION[name]','$hidden_perfoma_nos')";
