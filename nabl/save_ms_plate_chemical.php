...
if(isset($_POST['action_type']) && !empty($_POST['action_type']))
	{
		...
		{
			$get_query = "select * from `ms_plate_chemical` WHERE id='$_POST[id]' AND `is_deleted`='0'"; 
		...
		else if($_POST['action_type'] == 'add')
		{
			...
			$curr_date=date("Y-m-d");
			 ...
			 $insert="insert into `ms_plate_chemical` (`report_no`, `ulr`, `job_no`, `lab_no`, `status`, `created_by`, `created_date`, `modified_by`, `modified_date`, `is_deleted`, `deleted_by`, `checked_by`, `chk_dia`, `chk_elo`, `chk_ten`, `chk_yled`, `chk_thk`, `chk_mass`, `chk_bend`, `bend1`, `ms_grade`, `l1`, `w1`, `t1`, `out1`, `weight1`, `len1`, `mass1`, `dia1`, `width1`, `thk1`, `area1`, `load1`, `str1`, `ult1`, `ten1`, `initial1`, `final1`, `elo1`, `location1`, `tube_temp`, `tube_humidity`, `chk_chem`,`c1`,`c2`,`c3`,`c4`,`c5`,`c6`,`c7`,`c8`,`c9`,`c10`,`c11`,`c12`,`c13`,`c14`,`amend_date`) values('$report_no', '$ulr', '$job_no', '$lab_no', '$status', '$created_by', '$created_date', '$modified_by', '$modified_date', '$is_deleted', '$deleted_by', '$checked_by', '$chk_dia', '$chk_elo', '$chk_ten', '$chk_yled', '$chk_thk', '$chk_mass', '$chk_bend', '$bend1', '$ms_grade', '$l1', '$w1', '$t1', '$out1', '$weight1', '$len1', '$mass1', '$dia1', '$width1', '$thk1', '$area1', '$load1', '$str1', '$ult1', '$ten1', '$initial1', '$final1', '$elo1', '$location1', '$tube_temp', '$tube_humidity', '$chk_chem','$c1','$c2','$c3','$c4','$c5','$c6','$c7','$c8','$c9','$c10','$c11','$c12','$c13','$c14','$amend_date')"; 
											...
											<div class="col-lg-12">
												<table border="1px solid black" align="center" width="100%" id="aaaa">
													<tr>
														...
														<?php
													 $query = "select * from ms_plate_chemical WHERE lab_no='$lab_no' and `is_deleted`='0'";
		...
		$update="update `ms_plate_chemical` SET 
		`job_no`='$_POST[job_no]',
		`lab_no`='$_POST[lab_no]',
		`ulr`='$_POST[ulr]',		 
		`tube_humidity`='$_POST[tube_humidity]',		 
	...
	elseif($_POST['action_type'] == 'delete'){
		 ...
		 $delete="update `ms_plate_chemical` SET `is_deleted`='1',`deleted_by`='$_SESSION[name]' WHERE `report_no`='$_POST[report_no]' and `lab_no`='$_POST[lab_no]' and `job_no`='$_POST[job_no]'";
	...
	elseif($_POST['action_type'] == 'chk'){
		...
		$qry = "SELECT * FROM ms_plate_chemical WHERE `id`='$_POST[id]'";
		$arr = mysqli_query($conn,$qry);
		$rows1=mysqli_num_rows($arr);
		while($r2 = mysqli_fetch_array($arr)){
				if($r2['checked_by']!="")
				{
					$c1c="update ms_plate_chemical SET `checked_by`='' WHERE `id`='$_POST[id]'";			
					$result_of_delete1=mysqli_query($conn,$c1c);	
				}
				else
				{
					$cc="update ms_plate_chemical SET `checked_by`='$_SESSION[name]' WHERE `id`='$_POST[id]'";			
