...
$up_eng_light="update job_for_engineer set `biller_light_status`=1 where `trf_no`='$txt_trf_no' AND `job_no`='$txt_jobs' AND `biller_light_status`='0'";
								...
								<div class="box box-info">
									...
									if($set_status=="yes")
									{
										...
										<input type="hidden" name="testlist" class="form-control" id="testlist_<?php echo $material_count;?>" value="<?php echo $print_test; ?>">
											...
											<td>
											  <a href="<?php echo $base_url.$row_mat["filename"];?>?lab_no=<?php echo $row['lab_no']; ?>&&job_no=<?php echo $_GET['job_no']; ?>&&trf_no=<?php echo $_GET['trf_no']; ?>" class="btn btn-primary btn-lg btn3d" title="View Job"><span class="glyphicon glyphicon-question-list"></span> Report</a>
											&nbsp;
...
$(document).on("click", ".report_send_to_complete", function () {
	...
	$.ajax({
        ...
        success:function(html){
			window.location.href="<?php echo $base_url; ?>view_job_by_biller.php?trf_no="+txt_trf_no+"&&job_no="+txt_job_no;
