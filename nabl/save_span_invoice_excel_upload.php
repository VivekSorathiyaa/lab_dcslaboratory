...
if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
	...
	}else if($_POST['action_type'] == 'upload_excel')
	{
		...
		$test_mate_array= $_POST["test_mate_array"];
		    ...
		    //insert entry in estimate_sequence_maintain
			$todays=date("Y-m-d");
			$insert_sequence="insert into estimate_total_span_bill_sequence(`trf_no`,`job_no`,`bill_no`,`estimate_type`,`estimate_date`,`created_date`,`created_by`,`created_name`) values('$txt_trf_no','$txt_job_no','$set_bill_no','for_invoice_excel','$invoice_date','$todays','$_SESSION[u_id]','$_SESSION[name]')";
			$result_insert_sequence=mysqli_query($conn,$insert_sequence);
			
		   $insert_estimate="insert into estimate_total_span_only_for_test (`nabl_type`,`trf_no`,`job_no`,`estimate_no`,`bill_to`,`bill_no`,`gst_no`,`hsn_codes`,`estimate_date`,`agency_id`,`rate_type`,`gst_type`,`gst_in_or_ex`,`test_ids`,`test_name`,`test_qty`,`test_rates`,`test_totals`,`c_gst_amt`,`s_gst_amt`,`i_gst_amt`,`discount_percent`,`discount_amount`,`sub_total`,`grand_total`,`total_amt`,`total_amt_in_word`,`est_creatredby`,`est_createddate`,`est_modifyby`,`est_modifydate`,`mate_name`,`invoice_type`,`invoice_excel_upload`) 
						values(
						'nabl',
						'$txt_trf_no',
						'$txt_job_no',
