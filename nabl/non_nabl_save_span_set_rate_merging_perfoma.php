...
if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
    if($_POST['action_type'] == 'get_data_by_rate'){
		...
		// get test record by test id
					...
					$sel_test_query=mysqli_query($conn_of_non,$sel_test);
	...
	}else if($_POST['action_type'] == 'save_next_estimate_only_save')
	{
		...
		foreach($explode_trf as $keyed => $one_trf)
		{
			...
			$update_save_material_assign="update save_material_assign set `is_estimate`=1 where `trf_no`='$txt_trf_no' AND `job_no`='$txt_job_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyed]'";
			$result_save_material_assign=mysqli_query($conn_of_non,$update_save_material_assign);
			
			$get_job_data = "select * from job WHERE `trf_no`='$txt_trf_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyed]'";
			...
			if(mysqli_num_rows($resultset)>0)
			{
				...
				if($report_sent_to=="0")
				{
					$update_gst = "update job set client_gstno='$gst_no',`perfoma_completed_by_biller`=1 where `trf_no`='$txt_trf_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyed]'";				
				}
				else
				{
					$update_gst = "update job set agency_gstno='$gst_no',`perfoma_completed_by_biller`=1 where `trf_no`='$txt_trf_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyed]'";
				}
				$result_updategst=mysqli_query($conn_of_non,$update_gst);
			}
				
			// get morr by report no andjob no
			$sel_span_mate="select * from span_material_assign where `trf_no`='$txt_trf_no' AND `job_number`='$txt_job_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyed]'";
			...
			// code to get sample receve date from job table
			$sel_jobs="select * from job where `trf_no`='$txt_trf_no'AND `temporary_trf_no`='$explode_temporary_trf_no[$keyed]'";
			...
			$update_jobs="update job set `morr`='$get_morr',`job_lab_assign`='1',`job_lab_progress`='$j_n_progress',`report_job_printing`='$report_printing',`job_number`='$txt_job_no', `job_lab_progress_date`='$set_expected_date',`job_lab_progress_end_date`='$set_re_sample_date',`reported_by`='$reported_by',`admin_special_light`=3 where `trf_no`='$txt_trf_no'AND `temporary_trf_no`='$explode_temporary_trf_no[$keyed]'";
