...
if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
	...
	}else if($_POST['action_type'] == 'upload_excel')
	{
		...
		$test_mate_array= $_POST["test_mate_array"];
			...
			{
				$explode_bill_no=explode("/",$bill_no);
				$set_document_no=$explode_bill_no[2];
		    ...
		    //insert entry in estimate_sequence_maintain
			$todays=date("Y-m-d");
			$insert_sequence="insert into estimate_total_span_bill_sequence(`temporary_trf_no`,`trf_no`,`job_no`,`bill_no`,`estimate_type`,`estimate_date`,`created_date`,`created_by`,`created_name`) values('$txt_temporary_trf_no','$txt_trf_no','$txt_job_no','$set_bill_no','for_invoice_excel','$invoice_date','$todays','$_SESSION[u_id]','$_SESSION[name]')";
			$result_insert_sequence=mysqli_query($conn,$insert_sequence);
			
		   $insert_estimate="insert into estimate_total_span_only_for_test (`nabl_type`,`temporary_trf_no`,`trf_no`,`job_no`,`estimate_no`,`bill_to`,`bill_no`,`gst_no`,`hsn_codes`,`estimate_date`,`agency_id`,`rate_type`,`gst_type`,`gst_in_or_ex`,`test_ids`,`test_name`,`test_qty`,`test_rates`,`test_totals`,`c_gst_amt`,`s_gst_amt`,`i_gst_amt`,`discount_percent`,`discount_amount`,`sub_total`,`grand_total`,`total_amt`,`total_amt_in_word`,`est_creatredby`,`est_createddate`,`est_modifyby`,`est_modifydate`,`mate_name`,`invoice_type`,`invoice_excel_upload`) 
						values(
						'non_nabl',
	...
	else if($_POST['action_type'] == 'upload_direct_perfoma_excel')
	{
		$gst_no= $_POST["gst_no"];
		$hsn_codes= $_POST["hsn_codes"];
		$replace_date= str_replace("/","-",$_POST['invoice_date']);
		...
		$hidden_perfoma_no= $_POST["hidden_perfoma_no"];
			...
			//insert entry in estimate_sequence_maintain
			$todays=date("Y-m-d");
			$insert_sequence="insert into estimate_total_span_bill_sequence(`bill_no`,`estimate_type`,`estimate_date`,`created_date`,`created_by`,`created_name`,`perfoma_no`) values('$set_bill_no','direct_perfoma_excel','$invoice_date','$todays','$_SESSION[u_id]','$_SESSION[name]','$hidden_perfoma_no')";
			$result_insert_sequence=mysqli_query($conn,$insert_sequence);
			
		  $insert_estimate="insert into estimate_total_span_only_for_test (`bill_to`,`bill_no`,`gst_no`,`hsn_codes`,`estimate_date`,`agency_id`,`rate_type`,`gst_type`,`gst_in_or_ex`,`test_ids`,`test_name`,`test_qty`,`test_rates`,`test_totals`,`c_gst_amt`,`s_gst_amt`,`i_gst_amt`,`discount_percent`,`discount_amount`,`sub_total`,`grand_total`,`total_amt`,`total_amt_in_word`,`est_creatredby`,`est_createddate`,`est_modifyby`,`est_modifydate`,`mate_name`,`invoice_type`,`invoice_excel_upload`,`other_customer_name`,`other_customer_address`,`other_customer_gst_no`,`perfoma_no`) 
						values(
						'$txt_bill_to',
						'$set_bill_no',
						'$gst_no',
	...
	}else if($_POST['action_type'] == 'delete_invoice_excel')
	{
		
		$bill_no= $_POST["bill_no"];
		$del_perf="update estimate_total_span_only_for_test set `est_isdeleted`=1 where `bill_no`='$bill_no'";
		$delt_perfoma=mysqli_query($conn,$del_perf);
		$fill = array($delt_perfoma);
		
		
		$del_bill="update estimate_total_span_bill_sequence set `is_deleted`=1 where `bill_no`='$bill_no'";
		$delt_bill=mysqli_query($conn,$del_bill);
		$fill = array($delt_bill);
		echo json_encode($fill);
		
		
	}
	else if($_POST['action_type'] == 'delete_direct_perfoma_excel')
	{
