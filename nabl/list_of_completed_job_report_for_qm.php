...
<div class="row">
		<div class="row">
		<a href="list_of_completed_job_report_for_qm_for_edit.php" style=""></a>
		<h1 style="text-align:center;">
		View Completed Jobs
		</h1>
	</div>		
						...
						<div id="display_data_report">
								...
								<tbody>
									<?php
										...
										$query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `report_job_printing`='1' AND `accepted_by_qm`=1 AND `job_owner_eng_and_qm`=2 ORDER BY job_id DESC LIMIT 0,200";
									...
									?>
											...
											<!--<a href="edit_only_trf.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>&&temporary_trf_no=<?php echo $row['temporary_trf_no'];?>" class="btn btn-success " title="EDIT TRF"><span class="fa fa-tripadvisor"></span></a>
											<?php if($row['print_done_by_biller_for_qm_see']==1){ ?>
											<a href="download_data.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>&&temporary_trf_no=<?php echo $row['temporary_trf_no'];?>" class="btn btn-success" title="PRINT REPORT"><span class="fa fa-print"></span></a>&nbsp;
											<?php } 
											}
											?>-->
											</td>
...
$(document).on("click", ".view-report-btn", function () {
    ...
    $.ajax({
        ...
        success: function(response) {
                ...
                window.location.href = 'view_job_by_HA.php?trf_no=' + trf_no + '&&job_no=' + job_no + '&&temporary_trf_no=' + temporary_trf_no;
...
$(".search_job_by_qm").click(function()
{
	...
	if(search_trf_no =="" && search_sam_date =="")
	{
		...
		$.ajax({
			url : "<?php echo $base_url; ?>search_list_of_completed_job_report_for_qm.php",
			type: "POST",
			data : postData,
			beforeSend: function(){
