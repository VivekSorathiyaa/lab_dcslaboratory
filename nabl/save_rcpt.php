...
if (isset($_POST['action_type']) && !empty($_POST['action_type'])) {
	if ($_POST['action_type'] == 'data') {
		$get_query = "select * from rcpt WHERE id='$_POST[id]' AND `is_deleted`='0'";
		...
		$fill = array(
			...
			'casting_date' => date('Y-m-d', strtotime($result['casting_date'])),
		...
		$casting_date =  date('Y-m-d', strtotime($_POST['casting_date']));
		...
		$insert = "insert into rcpt (`report_no`, `ulr`, `job_no`, `lab_no`, `status`, `created_by`, `created_date`, `modified_by`, `modified_date`, `is_deleted`, `deleted_by`, `checked_by`, `chk_com`, `location1`, `location2`, `location3`, `location4`, `weight1`, `weight2`, `weight3`, `weight4`, `dia1`, `dia2`, `dia3`, `dia4`, `height1`, `height2`, `height3`, `height4`, `area1`, `area2`, `area3`, `area4`, `hd_ratio1`, `hd_ratio2`, `hd_ratio3`, `hd_ratio4`, `vol1`, `vol2`, `vol3`, `vol4`, `den1`, `den2`, `den3`, `den4`, `load1`, `load2`, `load3`, `load4`, `com1`, `com2`, `com3`, `com4`, `dia_corr1`, `dia_corr2`, `dia_corr3`, `dia_corr4`, `hd_corr1`, `hd_corr2`, `hd_corr3`, `hd_corr4`, `corr_com1`, `corr_com2`, `corr_com3`, `corr_com4`, `eq_cube1`, `eq_cube2`, `eq_cube3`, `eq_cube4`, `cip_1`, `cip_2`, `identification_mark`, `cube_grade`, `casting_date`, `amend_date`) values('$report_no','$ulr','$job_no','$lab_no','0','$_SESSION[name]','$curr_date','','0000-00-00','0','','', '$chk_com', '$location1', '$location2', '$location3', '$location4', '$weight1', '$weight2', '$weight3', '$weight4', '$dia1', '$dia2', '$dia3', '$dia4', '$height1', '$height2', '$height3', '$height4', '$area1', '$area2', '$area3', '$area4', '$hd_ratio1', '$hd_ratio2', '$hd_ratio3', '$hd_ratio4', '$vol1', '$vol2', '$vol3', '$vol4', '$den1', '$den2', '$den3', '$den4', '$load1', '$load2', '$load3', '$load4', '$com1', '$com2', '$com3', '$com4', '$dia_corr1', '$dia_corr2', '$dia_corr3', '$dia_corr4', '$hd_corr1', '$hd_corr2', '$hd_corr3', '$hd_corr4', '$corr_com1', '$corr_com2', '$corr_com3', '$corr_com4', '$eq_cube1', '$eq_cube2', '$eq_cube3', '$eq_cube4', '$cip_1', '$cip_2', '$identification_mark', '$cube_grade', '$casting_date', '$amend_date')";
		
		$result_of_insert = mysqli_query($conn, $insert);

		$query = "select * from rcpt WHERE lab_no='$lab_no' AND job_no='$job_no' and `is_deleted`='0'";
...
<?php
		...
		$lab_no = $_POST['lab_no'];
		$job_no = $_POST['job_no'];
		$delete = "update rcpt SET `is_deleted`='1', `deleted_by`='$_SESSION[name]' WHERE `id`='$_POST[id]'";
		...
		$qry = "SELECT * FROM rcpt WHERE `id`='$_POST[id]'";
		$arr = mysqli_query($conn, $qry);
		$rows1 = mysqli_num_rows($arr);
		while ($r2 = mysqli_fetch_array($arr)) {
			if ($r2['checked_by'] != "") {
				$c1c = "update rcpt SET `checked_by`='' WHERE `id`='$_POST[id]'";
				$result_of_delete1 = mysqli_query($conn, $c1c);
			} else {
				$cc = "update rcpt SET `checked_by`='$_SESSION[name]' WHERE `id`='$_POST[id]'";
