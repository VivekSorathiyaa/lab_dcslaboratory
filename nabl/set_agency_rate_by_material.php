...
$up_rate_one="update agency_rate_by_material set `is_active`='1' where `agency_id`='$achive_agency'";
$query_up_rate_one=mysqli_query($conn,$up_rate_one);
}

$sel_agency_tempalte="select * from agency_rate_by_material where `is_deleted`=0 ORDER BY `agency_rate_id` DESC";
...
if(mysqli_num_rows($query_rtemplate) >0){
	...
	$set_template_id= intval($get_template["template_no"]) + 1;
	
}else{
	$set_template_id=1;
}

$dele="delete from  agency_rate_by_material where `is_active`='0'";
...
function search_agency(type,id){
    ...
    $.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>save_agency_rate_by_material.php',
        data: billData,
		beforeSend: function(){
...
$(document).on("blur", ".change_rate_class", function () {
	...
	var agency_rate_id= $(this).attr("data-id");
	
	$.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>save_agency_rate_by_material.php',
        data: 'action_type=set_rate_on_change&set_rates='+set_rates+'&agency_rate_id='+agency_rate_id,
		beforeSend: function(){
		document.getElementById("overlay_div").style.display="block";
		},
        success:function(html){
...
$(document).on("click", ".delete_all_template", function () {
	...
	$.ajax({
        ...
        success:function(html){
		document.getElementById("overlay_div").style.display="none";	
			window.location.href="<?php echo $base_url; ?>set_agency_rate_by_material.php";
        }
    });
