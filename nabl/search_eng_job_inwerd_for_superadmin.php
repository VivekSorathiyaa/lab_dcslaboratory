...
if($_SESSION['name']=="")
{
	...
	<?php
		...
		if($from_date!= "" && $to_date!= ""){
			$where="And `jobcreateddate` >= '$new_from_date' AND `jobcreateddate` <='$new_to_date' ";
		}

		else if($to_date!= "" ){
			$where="And `jobcreateddate` <='$new_to_date'  ";
		}

		else if($from_date!= "" ){
			$where="And `jobcreateddate` >= '$new_from_date' ";
		}
					...
					<table id="example1" class="table table-bordered table-striped" style="width:100%">
								...
								<tbody>
									<?php
										...
										$count=0;
										$query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `morr`= 'r' AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `tested_by`='$user_id' AND `report_job_printing`='1' AND `job_sent_to_qm`='0'  ".$where." ORDER BY job_id DESC";
									...
									?>
											...
											<td style="white-space:nowrap;">
												...
												<a href="javascript:void(0);" class="btn btn-danger btn-lg btn3d" title="DELETE JOB"onclick="return confirm('Are you sure to delete data?')?deleteData('delete','<?php echo $row['report_no']; ?>'):false;"><span class="glyphicon glyphicon-question-ok"></span>DELETE JOB</a>
