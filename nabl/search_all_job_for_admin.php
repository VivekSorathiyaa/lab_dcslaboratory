...
if($get_agency_ids != "" && $sel_clients_ids != "" && $sel_auth_names != "" && $agree_nos != "")
{
	...
	{
		$agency_connect .="`agency`='$one_agency'";
	...
	{
		$clients_connect .="`client_code`='$one_clients'";
	...
	{
		$auth_connect .="`person_name`='$one_auth'";
	...
	//explode agree
	$explode_agree=explode(",",$agree_nos);
	$agree_connect="";
	$agree_counts=count($explode_agree);
	$count_down_agree=1;
	foreach($explode_agree as $one_agree)
	{
		$agree_connect .="`agreement_no` LIKE '$one_agree'";
		if($agree_counts != $count_down_agree)
		{
		$agree_connect .=" OR ";			
		}
		$count_down_agree++;
	...
	$wheres="`jobisdeleted`=0 AND ".$agency_connect." AND ".$clients_connect." AND ".$auth_connect." AND ".$agree_connect;
}
// not agency
if($get_agency_ids == "" && $sel_clients_ids != "" && $sel_auth_names != "" && $agree_nos != "")
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$clients_connect." AND ".$auth_connect." AND ".$agree_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$agency_connect."  AND ".$auth_connect." AND ".$agree_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$agency_connect." AND ".$clients_connect." AND ".$agree_connect;
}
// not agree
if($get_agency_ids != "" && $sel_clients_ids != "" && $sel_auth_names != "" && $agree_nos == "")
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$agency_connect." AND ".$clients_connect." AND ".$auth_connect;
}
// only agency
if($get_agency_ids != "" && $sel_clients_ids == "" && $sel_auth_names == "" && $agree_nos == "")
{	
	...
	$wheres="`jobisdeleted`=0 AND $agency_connect";
}
// only client
if($get_agency_ids == "" && $sel_clients_ids != "" && $sel_auth_names == "" && $agree_nos == "")
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$clients_connect;
}
// only auth
if($get_agency_ids == "" && $sel_clients_ids == "" && $sel_auth_names != "" && $agree_nos == "")
{	
	...
	$wheres="`jobisdeleted`=0 AND $auth_connect";
}
// only agree
if($get_agency_ids == "" && $sel_clients_ids == "" && $sel_auth_names == "" && $agree_nos != "")
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$agree_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$agency_connect." AND ".$clients_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$agency_connect." AND ".$auth_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$agency_connect." AND ".$agree_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$clients_connect." AND ".$auth_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$clients_connect." AND ".$agree_connect;
}
...
{	
	...
	$wheres="`jobisdeleted`=0 AND ".$auth_connect." AND ".$agree_connect;
}
...
// only name_of_work
if($get_agency_ids == "" && $sel_clients_ids == "" && $sel_auth_names == "" && $agree_nos == "" && $name_of_work != "")
{	
	$wheres="`jobisdeleted`=0 AND `nameofwork` LIKE '%$name_of_work%'";
}
...
<table id="example1" class="table table-bordered table-striped" style="width:100%;">
									...
									<?php
										...
										$count=0;
									  $query="select * from job where ".$wheres;
									...
									?>
											...
											<?php $sel_agency="select * from agency_master where `isdeleted`=0 where `agency_id`=".$row["agency"];
											...
											<td>
											<?php 
											 $sel_bill="select * from estimate_total_span_only_bill where `est_isdeleted`=0 AND `report_no`='$reports_no'";
											 $query_sel_bill = mysqli_query($conn, $sel_bill);
											 if(mysqli_num_rows($query_sel_bill)>0)
											 {
												 $result_sel_bill = mysqli_fetch_array($query_sel_bill);
												 echo $result_sel_bill["total_amt"];
											 }else
											 {
												$sel_esti="select * from estimate_total_span where `est_isdeleted`=0 AND `report_no`='$reports_no'";
												$query_sel_esti = mysqli_query($conn, $sel_esti);
												if(mysqli_num_rows($query_sel_esti)>0)
												{
													$result_sel_esti = mysqli_fetch_array($query_sel_esti);
													echo $result_sel_esti["total_amt"];
												}else
												{
													echo 00;
												}
    ...
    var table = $('#example1').DataTable( {
      ...
      var api = this.api();
       ...
       // var pageTotal=api.column( 17, {page:'every'} ).data().sum();
		...
		$('tr:eq(0) td:eq(10)', api.table().footer()).html("TOTAL="+Math.round(paid_total+remain_total)+"   <br>PAID=  "+paid_total+"      <br>UNPAID="  +remain_total);
