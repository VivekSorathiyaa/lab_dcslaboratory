						...
						<div id="display_data_report">
								<table id="example2" class="table table-bordered table-striped" style="width:100%;">
									...
									<tr>
										...
										<th style="text-align:center;width:5%;">Reporting Date</th>
										
										<th style="text-align:center;width:5%;">Material</th>
								...
								<tbody>
									<?php
									    ...
									    $query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `report_job_printing`='1' AND `appoved_by_qm_to_print`=0 AND `any_report_done_by_any_eng`=1 AND `accepted_by_qm`=0 ORDER BY job_id DESC";
										...
										while($row=mysqli_fetch_array($result))
										{
											...
											$explode_reported_by=explode(",",$row["reported_by_authorize"]);
											
											if (in_array($_SESSION['u_id'], $explode_reported_by))
											{
									...
									?>
											...
											<a href="view_job_by_authority.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>&&temporary_trf_no=<?php echo $row['temporary_trf_no'];?>" class="btn btn-primary" title="View Job"><span class="fa fa-eye"></span></a>
											...
											{ ?>
											   <!--<a href="download_data.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>" class="btn btn-warning" title="Print"><span class="fa fa-print"></span></a>&nbsp;-->
											<?php } ?>
											
											<!--<a href="javascript:void(0);" class="btn btn-success job_send_to_accept <?php //echo $disabling;?>" data-id="<?php //echo $row['trf_no']."|".$row['job_number'];?>"title="Material Assign"><span class="fa fa-arrow-circle-right"></span></a>-->
											
											<?php }else{ ?>
											
											<a href="javascript:void(0);" class="btn btn-warning job_send_to_print" data-id="<?php echo $row['trf_no']."|".$row['job_number'];?>"title="Material Assign"><span class="fa fa-print"></span></a>
											...
											<td style="white-space:nowrap;text-align:center;"><?php echo $reporting_dates;?></td>
												...
												<span style="visibility: hidden;"><?php echo $row["agency_name"];?></span>
											</td>
											<td style="white-space:nowrap;text-align:center;">
												<span style="visibility: hidden;"><?php echo $row["clientname"];?></span>
											</td>
											<td style="white-space:nowrap;text-align:center;">
												<span style="visibility: hidden;"><?php echo $row["tpi_name"];?></span>
											</td>
											<td style="white-space:nowrap;text-align:center;">
												<span style="visibility: hidden;"><?php echo $row["pmc_name"];?></span>
											</td>-->
											...
											?></td>
											<!--<td style="white-space:nowrap;text-align:center;">
												<span style="visibility: hidden;"><?php echo $row["refno"];?></span>
											</td>
											<td style="white-space:nowrap;text-align:center;">
												<span style="visibility: hidden;"><?php echo $row["nameofwork"];?></span>
											</td>-->
										</tr>
									<?php
											}
