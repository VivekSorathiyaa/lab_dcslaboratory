	...
	<table id="example2" class="table table-bordered table-striped" style="width:100%;">
								...
								<tbody>
									<?php
										...
										$query="select * from job WHERE `jobisdeleted`=0 AND `assign_status`=1 AND `send_to_second_reception`=1 AND `material_assign`= 1 AND `job_lab_assign`='1' AND `job_lab_progress`= '1' AND `report_job_printing`='1' AND `job_owner_eng_and_qm`=2 ".$where." ORDER BY job_id DESC";
									...
									?>
											...
											{
											?>
											<a href="view_job_by_qlty_manager.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>" class="btn btn-primary" title="VIEW JOB"><span class="fa fa-eye"></span></a>
											
											<a href="edit_only_trf.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>" class="btn btn-success " title="EDIT TRF"><span class="fa fa-tripadvisor"></span></a>
											<?php if($row['print_done_by_biller_for_qm_see']==1){ ?>
											<a href="download_data.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>" class="btn btn-success" title="PRINT REPORT"><span class="fa fa-print"></span></a>&nbsp;
											<?php } 
											}
											?>
											</td>
