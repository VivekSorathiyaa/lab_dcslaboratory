                                    ...
                                    <div class="table-responsive" id="display_data">
                                        <table class="table table-bordered no-margin">
                                            ...
                                            <tbody>
                                                    ...
                                                    while ($row = mysqli_fetch_array($result)) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
																...
																<input type="file" class="btn-primary form-control uplodings" id="<?php echo str_replace("/","_",$row['lab_no']).'|'.$row['final_material_id'];?>" style="width: 35px;font-size: 18px;" multiple >
																<?php }else{ 
																?>
																...
																<input type="button" value="Notes" class="btn btn-info btn_witness" data-id="<?php echo $row['final_material_id'];?>" data-toggle="modal" data-target="#modal-witness" style="margin-top:20px;">
																
																		<?php if($row["obs_by_eng"]=="1" && $row["tm_verify"]=="0"){ ?>
																		 <a href="javascript:void(0);" class="btn btn-success btn-lg btn3d verified_by_tm" data-id="<?php echo $row['lab_no'].'|'.$row['trf_no'].'|'.$row['final_material_id']; ?>" title="Report Send To Quality Managetr"><span class="glyphicon glyphicon-question-list"></span> Submit</a>
																		<?php }
																		else if($row["obs_by_eng"]=="0" && $row["tm_verify"]=="0"){
																			echo '<span style="color:red;font-weight:bold;">Upload Pending</span>';
																		}
																		else{ echo '<span style="color:green;font-weight:bold;">Verified</span>';}?>
                                                ...
                                                $select_job_for_eng = "select * from job_for_engineer where `tm_verify`=0 AND `trf_no` = '$get_trf_no'";
                                                $query_job_for_eng = mysqli_query($conn, $select_job_for_eng);
                                                if (mysqli_num_rows($query_job_for_eng) == 0) {
                                                    
													$upd_jobs="update job set `all_verify`='1' where `trf_no`='$get_trf_no'";
...
<script>
    ...
    $(document).on("click", ".verified_by_tm", function() {
        var clicked_id = $(this).attr("data-id");
        $.confirm({
            title: "warning",
            content: "Are You Sure To Verified?",
            buttons: {
                confirm: function() {
                    $.ajax({
                        type: 'POST',
                        url: '<?php $base_url; ?>save_span_engineer.php',
                        data: 'action_type=verified_by_tm&clicked_id=' + clicked_id,
