...
if($_SESSION['name']=="")
{
	...
	$txt_temporary_trf_no= $get_temporary_trf_no[0];
...
$select_test_wise="select * from test_wise_material_rate where `trf_no`='$txt_trf_no' AND `job_no`='$txt_jobs' AND `temporary_trf_no`='$txt_temporary_trf_no'";
 $select_test_wise_query=mysqli_query($conn_of_non,$select_test_wise);

 // get_estimate if available
	$sel_estiamte="select * from estimate_total_span_only_for_test where `trf_no`='$_GET[chk_array]' AND `job_no`='$_GET[chk_array]'  AND `est_isdeleted`=0 AND `nabl_type`='non_nabl' AND `temporary_trf_no`='$_GET[temporary_trf_no]'";
 ...
 if(mysqli_num_rows($estiamte_query) > 0){
	 ...
	 $test_totals= explode(",",$get_estimate["test_totals"]);
		...
		if(mysqli_num_rows($perfoma_query) > 0)
		{
		   ...
		   $merge_material_ids_array=explode(",",$get_perfoma["mat_ids"]);
	 ...
	 $bill_no="";
		...
		{
			$sel_jobs_doc="select * from `job` where `trf_no`='$one_chk_array' AND `temporary_trf_no`='$get_temporary_trf_no[$keyed]'";
					...
					<div class="box box-info">
										  ...
										  <?php
											...
											if($is_estimate_avail=="not_available")
											{
														foreach($merge_test_id_array as $keyer => $one_test_id)
														  ...
														  <tr>
														  <td colspan="4">
														  <input type="hidden" name="mt_id[]" class="class_mate_id" id="mateid_<?php echo $keyer;?>" value="<?php echo $merge_material_ids_array[$keyer]; ?>">
										  ...
										  if($is_estimate_avail=="available")
											{
												...
												{ ?>
													...
													<tr>
													 <td colspan="4">
													 <input type="hidden" name="mt_id[]" class="class_mate_id" id="mateid_<?php echo $keyer;?>" value="<?php echo $mate_ids[$one_key]; ?>">
							...
							<input type="hidden" name="hidden_agency" id="hidden_agency" value="<?php echo $get_agency_id;?>">
								<div class="row">
									...
									<a href="non_nabl_matt_invoice_bill_by_test_print.php?chk_array=<?php echo $get_estimate['trf_no'];?>&&temporary_trf_no_array=<?php echo $get_estimate['temporary_trf_no'];?>" class="btn btn-info" title="" target="_blank" style="width:150px;font-size:20px;"><span class="glyphicon glyphicon-question-list"></span> Print</a>
...
$("#select_rate").change(function(){
      ...
      var postData = 'action_type=get_data_by_rate&select_rate='+select_rate+'&test_id_array='+test_id_array+'&test_qty_array='+test_qty_array;
		  ...
		  return false;
			...
			$.ajax({
				url : "<?php $base_url; ?>non_nabl_save_span_set_rate_for_test_merging.php",
				type: "POST",
				data : postData,
				dataType:'JSON',
		...
		if(hidden_gst_type !="without_gst")
		{
			...
			else
			{
				...
				success:function(msg){
					...
					window.location.href="<?php echo $base_url; ?>non_nabl_span_set_rate_only_by_test_merging.php?chk_array="+txt_trf_no+"&&temporary_trf_no="+txt_temporary_trf_no;
