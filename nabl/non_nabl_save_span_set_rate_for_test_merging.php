...
if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
	...
	}else if($_POST['action_type'] == 'save_next_estimate_only_save')
	{
		...
		$sel_estimate="select * from estimate_total_span_only_for_test where `trf_no`='$txt_trf_no' AND `job_no`='$txt_job_no' AND `temporary_trf_no`='$txt_temporary_trf_no' AND `nabl_type`='non_nabl'";
		$result_estimate=mysqli_query($conn,$sel_estimate);
		
		if(mysqli_num_rows($result_estimate) > 0){
			
	    $update_est="update estimate_total_span_only_for_test set `estimate_no`='$txt_invoice_no',`bill_to`='$txt_bill_to',`gst_no`='$gst_no',`hsn_codes`='$hsn_codes',`estimate_date`='$invoice_date',`agency_id`='$hidden_agency',`rate_type`='$select_rate',`gst_type`='$hidden_gst_type',`gst_in_or_ex`='$hidden_gst_in_ex',`c_gst_amt`='$txt_cgst',`s_gst_amt`='$txt_cgst',`i_gst_amt`='$txt_igst',`grand_total`='$txt_grand',`total_amt`='$total_amt',`total_amt_in_word`='$txt_amt_in_word',`sub_total`='$txt_sub_total',`discount_percent`='$txt_discount_percent',`discount_amount`='$txt_discount_amount',`mat_ids`='$test_mate_id_array',`test_ids`='$test_ids_array',`test_name`='$test_name_array',`test_qty`='$test_qty_array',`test_rates`='$test_rates_array',`test_totals`='$test_amnt_array',`mate_name`='$test_mate_array',`other_customer_name`='$other_customer_name',`other_customer_address`='$other_customer_address',`other_customer_gst_no`='$other_customer_gst_no' where `trf_no`='$txt_trf_no' AND `job_no`='$txt_job_no' AND `temporary_trf_no`='$txt_temporary_trf_no'";
		$result_estimate=mysqli_query($conn,$update_est);
		
		$up_perfomas="update estimate_total_span set `make_test_bill`='1',`make_material_bill`='0',`make_estimate`='0' where `perfoma_no`='$hidden_perfoma_nos'";
		...
		}else
		{
			...
			$todays=date("Y-m-d");
			$insert_sequence="insert into estimate_total_span_bill_sequence(`temporary_trf_no`,`trf_no`,`job_no`,`bill_no`,`estimate_type`,`estimate_date`,`created_date`,`created_by`,`created_name`) values('$txt_temporary_trf_no','$txt_trf_no','$txt_job_no','$set_bill_no','for_test','$invoice_date','$todays','$_SESSION[u_id]','$_SESSION[name]')";
		   ...
		   $insert_estimate="insert into estimate_total_span_only_for_test (`nabl_type`,`temporary_trf_no`,`trf_no`,`job_no`,`estimate_no`,`bill_to`,`bill_no`,`gst_no`,`hsn_codes`,`estimate_date`,`agency_id`,`rate_type`,`gst_type`,`gst_in_or_ex`,`mat_ids`,`test_ids`,`test_name`,`test_qty`,`test_rates`,`test_totals`,`c_gst_amt`,`s_gst_amt`,`i_gst_amt`,`discount_percent`,`discount_amount`,`sub_total`,`grand_total`,`total_amt`,`total_amt_in_word`,`est_creatredby`,`est_createddate`,`est_modifyby`,`est_modifydate`,`mate_name`,`invoice_type`,`chequeno`,`bank_name`,`tds`,`paid_amt`,`remarks`,`cheque_amt`,`perfoma_no`,`other_customer_name`,`other_customer_address`,`other_customer_gst_no`) 
						values(
						'non_nabl',
						...
						//set status for make test bill in perfoma table
						 $up_perfomas="update estimate_total_span set `make_test_bill`='1',`make_material_bill`='0',`make_estimate`='0' where `perfoma_no`='$hidden_perfoma_nos'";
						 $up_query_perfomas = mysqli_query($conn, $up_perfomas);
		}
			$explode_trf=explode(",",$txt_trf_no);
			$explode_temporary_trf_no=explode(",",$txt_temporary_trf_no);
			foreach($explode_trf as $keyies => $one_trf)
			{
				$txt_trf_no=$one_trf;
				$txt_job_no=$one_trf;
				
				$update_save_material_assign="update save_material_assign set `is_estimate`=1 where `trf_no`='$txt_trf_no' AND `job_no`='$txt_job_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyies]'";
				...
				if(mysqli_num_rows($resultset)>0)
				{
					...
					if($report_sent_to=="0")
					{
						$update_gst = "update job set client_gstno='$gst_no',`perfoma_completed_by_biller`=1 where `trf_no`='$txt_trf_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyies]'";				
					}
					else
					{
						$update_gst = "update job set agency_gstno='$gst_no',`perfoma_completed_by_biller`=1 where `trf_no`='$txt_trf_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyies]'";
					}
					$result_updategst=mysqli_query($conn_of_non,$update_gst);
				}
				// get morr by report no andjob no
				$sel_span_mate="select * from span_material_assign where `trf_no`='$txt_trf_no' AND `job_number`='$txt_job_no' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyies]'";
