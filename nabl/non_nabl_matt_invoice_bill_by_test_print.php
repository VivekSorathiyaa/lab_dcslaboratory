...
<?php
		...
		$temporary_trf_no_array=$_GET["temporary_trf_no_array"];
		$explode_temporary_trf_no=explode(",",$_GET["temporary_trf_no_array"]);
		$one_temporary_trf_no=$explode_temporary_trf_no[0];

		echo $sel_estiamte="select * from estimate_total_span_only_for_test where `trf_no`='$chk_array' AND `job_no`='$chk_array' AND `nabl_type`='non_nabl' AND `temporary_trf_no`='$temporary_trf_no_array'";
		...
		if($bill_to=="1"){
			...
			$result_city =mysqli_query($conn_of_non,$sel_city);
		...
		$result_tax =mysqli_query($conn_of_non,$sel_tax);
		...
		$refno="";
	  foreach($explode_trf_no as $keyess => $one_trf_nos)
	  {
		  $sel_job="select * from job where `trf_no`='$one_trf_nos' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyess]'";
		  $result_job =mysqli_query($conn_of_non,$sel_job);
		  $row_job =mysqli_fetch_array($result_job);
		  $refno .= '<span style="min-width:100px;">'.$row_job["refno"].'</span>'."&nbsp;&nbsp;&nbsp;&nbsp;Date:-&nbsp;&nbsp;".date('d-m-Y',strtotime($row_job["date"]))."<br>";

		  $update_jobs= "update job set `print_done_by_biller_for_qm_see`=1 where `trf_no`='$one_trf_nos' AND `temporary_trf_no`='$explode_temporary_trf_no[$keyess]'";
	...
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
		...
		<?php
				...
				foreach($test_name_array as $keys => $one_test_name)
				{
					...
					$query_test_category=mysqli_query($conn_of_non,$sele_test_categorys);
