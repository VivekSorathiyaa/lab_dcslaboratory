					...
					<div class="box-body">
								...
								<div id="display_data">
									...
									<tbody>
										<?php
											$count=0;
											$query="select * from span_bill WHERE `isdeleted`=0 AND `division_status`=1 AND `ispaid`=0 ORDER BY id DESC";
										...
										?>
												...
												<a href="javascript:void(0);" class="glyphicon glyphicon-question-ok reward_to_main" data-id="<?php echo $row['id'];?>"title="Reward Bill"><i class="fa fa-undo"></i> </a>
												
												<a href="javascript:void(0);" class="send_to_division" data-id="<?php echo $row['id'];?>"title="Send To Division"><i class="fa fa-send-o"></i> </a>
...
$(document).on("click", ".reward_to_main", function () {
				var clicked_id = $(this).attr("data-id");  
				
    
	$.confirm({
        title: "warning",
        content: "Are You Sure To Reward This Bill To Main List?",
        buttons: {
			confirm: function () {
	$.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>span_billing_save.php',
        data: 'action_type=reward_to_main&clicked_id='+clicked_id,
        success:function(html){
			window.location.href="<?php echo $base_url; ?>span_billing_listing.php";
        }
    });
...
$(document).on("click", ".send_to_division", function () {
				var clicked_id = $(this).attr("data-id");  
				
    
	$.confirm({
        title: "warning",
        content: "Are You Sure To Send This Bill To Division?",
        buttons: {
			confirm: function () {
	$.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>span_billing_save.php',
        data: 'action_type=send_to_division&clicked_id='+clicked_id,
        success:function(html){
			window.location.href="<?php echo $base_url; ?>span_billing_listing.php";
        }
    });
