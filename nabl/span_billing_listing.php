							...
							<div class="box-body">
								...
								<div id="display_data">
									...
									<tbody>
										<?php
											$count=0;
											$query="select * from span_bill WHERE `isdeleted`=0 AND `division_status`=0 AND `ispaid`=0 ORDER BY id DESC";
										...
										?>
												...
												<a href="javascript:void(0);" class="send_to_sub" data-id="<?php echo $row['id'];?>"title="Send To Sub Division"><i class="fa fa-send-o"></i> </a>
...
$(document).on("click", ".send_to_sub", function () {
				var clicked_id = $(this).attr("data-id");  
				
    
	$.confirm({
        title: "warning",
        content: "Are You Sure To Send This Bill To Sub Division?",
        buttons: {
			confirm: function () {
	$.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>span_billing_save.php',
        data: 'action_type=send_to_sub_division&clicked_id='+clicked_id,
        success:function(html){
			window.location.href="<?php echo $base_url; ?>span_billing_listing.php";
        }
    });
