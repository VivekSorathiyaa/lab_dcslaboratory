...
require 'D:\WFGS\htdocs\rajkotlab\nabl\PHPMailer-master\src/SMTP.php';
	...
	$invoice_no=$onces[0];
	$trf_nos=$onces[1];
		$sel_scaned="select * from upload_invoice where `is_deleted`=0 AND `invoice_no`='$invoice_no'";
			...
			if($agency_email!="")
			{
						...
						if (!in_array($agency_email, $mailing_array))
					{
						...
						if($in_mb< 9.9)
						{
							...
							if (!in_array($invoice_no, $trf_array))
							{
							array_push($trf_array,array($invoice_no));
							}
						}else{
							break;
						}
					...
					else
					{
						...
						if($in_mb < 9.9)
						{
							...
							if($set_size< 9.9)
							{
								...
								$sizing[$key]=$set_size;
								if (!in_array($invoice_no, $trf_array[$key]))
								{
								array_push($trf_array[$key],$invoice_no);
								}
							}else{
								break;
							}
	...
	if(!empty($mailing_array))
	{
		foreach($mailing_array as $mail_key => $one_mail)
		{
			$mail=new PHPMailer();
					...
					$mail->Subject   = "INVOICE "."(".implode(",",$trf_array[$mail_key]).")";
					$concats ="";
					$array_report=array();
					...
					{
						$mail->addAttachment('D:\WFGS\htdocs\rajkotlab\nabl\invoice_pdf_upload/'.$one_attech, $one_attech);
					}
					...
					if(!$mail->send()){
						...
						if(!empty($array_report))
						{
							...
							{
								$invoice_nos=substr($one_reports,1);
								$up_scan="update estimate_total_span set `is_invoice_mail`='1' WHERE `invoice_no`='$invoice_nos'";
