...
if (isset($_POST['action_type']) && !empty($_POST['action_type'])) {
	if ($_POST['action_type'] == 'data') {
		$get_query = "select * from concrete_core WHERE id='$_POST[id]' AND `is_deleted`='0'";
...
<?php
		...
		$update = "update concrete_core SET 
		...
		$lab_no = $_POST['lab_no'];
		$job_no = $_POST['job_no'];
		$delete = "update concrete_core SET `is_deleted`='1', `deleted_by`='$_SESSION[name]' WHERE `id`='$_POST[id]'";
		...
		$qry = "SELECT * FROM concrete_core WHERE `id`='$_POST[id]'";
		$arr = mysqli_query($conn, $qry);
		$rows1 = mysqli_num_rows($arr);
		while ($r2 = mysqli_fetch_array($arr)) {
			if ($r2['checked_by'] != "") {
				$c1c = "update concrete_core SET `checked_by`='' WHERE `id`='$_POST[id]'";
				$result_of_delete1 = mysqli_query($conn, $c1c);
			} else {
				$cc = "update concrete_core SET `checked_by`='$_SESSION[name]' WHERE `id`='$_POST[id]'";
