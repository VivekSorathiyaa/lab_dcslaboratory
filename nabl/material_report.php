...
<?php
				...
				if(isset($_POST['btn_estimate'])){
					...
					$gst_type_to_save=$_POST["gst_type_to_save"];
					$direct_path_to_save=$_POST["direct_path_to_save"];
					$report_no_to_save=$_POST["report_no_to_save"];
					$date_to_save=$_POST["date_to_save"];
					
					$insert_job_total="insert into job_id_to_material_total (`gst_type`,`report_number`,`date`,`direct_path`,`createdby_id`,`createdby`,`createddate`,`modifiedby`,`modifieddate`,`assign_status`,`isdeleted`)
					values(
					'$gst_type_to_save',
					'$report_no_to_save',
					'$date_to_save',
					...
					$result_of_insert=mysqli_query($conn,$insert_job_total);
									...
									<div class="col-lg-12">
										<div class="form-group">
											...
											<form name="main_form" method="post">
												...
												<button type="submit" class="btn btn-info pull-right" id="btn_estimate" name="btn_estimate" tabindex="20" style="width:100px">Estimate</button>
												</div>
											</form>
												<div class="col-xs-2">
...
function getbills(){
    ...
    $.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>addData.php',
        data: 'action_type=view_after_add_data&report_no=<?php echo $_GET["report_no"]; ?>',
        success:function(html){
			$('#display_data').html("");
            $('#display_data_for_edit').html("");
