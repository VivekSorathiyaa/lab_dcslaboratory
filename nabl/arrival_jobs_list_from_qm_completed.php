				...
				<!-- /.box-header -->
						...
						<div id="display_data_report">
								...
								<tbody>
									<?php
										$count=0;
										$query="select * from job WHERE `jobisdeleted`=0 AND `rec_to_tm`='1' AND `tm_to_eng_blank`='1' AND `any_upload_come`='1' AND `any_report_done_by_any_qm`='1' AND `all_accept_by_tm`='0'  ORDER BY job_id DESC";
									...
									?>
											...
											{
											?>
											<a href="view_job_by_tm_of_sa.php?trf_no=<?php echo $row['trf_no'];?>&&job_no=<?php echo $row['job_number'];?>&&temporary_trf_no=<?php echo $row['temporary_trf_no'];?>" class="btn btn-primary" title="View Job"><span class="fa fa-eye"></span></a>
											...
											$sel_span_table="select `lab_no` from span_material_assign where `isdeleted`=0 AND `trf_no`='$row[trf_no]'";
