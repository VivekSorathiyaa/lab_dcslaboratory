...
if(isset($_POST['update'])){
	...
	$update="update multi_login SET `staff_fullname`='$_POST[fullname]',`staff_dob`='$new_dob_date',`staff_email`='$_POST[email]',`staff_gender`='$_POST[gender]',`staff_address`='$_POST[address]',`staff_contactno`='$_POST[contact]',`staff_modifiedby`='$_SESSION[name]',`staff_modifieddate`='$curr_date' WHERE `id`='$uid'";
	
	if(isset($_FILES["profile_image"]) && $_FILES["profile_image"]["error"] == 0){
	$target_dir = "uplode/";
	$target_file = $target_dir . basename($_FILES["profile_image"]["name"]);
	
	$pro_image_name= $target_dir.$uid.".jpg";
...
// if everything is ok, try to upload file
	...
	if ($result_of_update) {
		...
		window.location.href="<?php echo $base_url; ?>master_dashboard.php";
 ...
 if(isset($_POST['change_pass'])){
	...
	$query="SELECT * FROM multi_login WHERE `id`='$_GET[uid]'";
	
		$qrys = mysqli_query($conn,$query);
		$r = mysqli_fetch_array($qrys);
		if($r['staff_first_pass']==$_POST['old_pass']){
		
			if($_POST['new_pass']==$_POST['con_pass']){
				
				$curr_date=date("Y-m-d");

				$update_pass="update multi_login SET `staff_first_pass`='$_POST[new_pass]',`staff_modifiedby`='$_SESSION[name]',`staff_modifieddate`='$curr_date' WHERE `id`='$_GET[uid]'";
				$result_of_update=mysqli_query($conn,$update_pass);	
				if ($result_of_update) {
				?>
					<script>alert("Change Password Sucessfully");
					 window.location.href="<?php echo $base_url; ?>master_dashboard.php";
			...
			?>
			<script>alert("New Password and Confirm Password are must same");</script>
			<?php
		}else{
			?>
			  ...
			  <!-- Profile Image -->
				<div class="box box-primary">
					<div class="box-body box-profile">
						<img class="profile-user-img img-responsive img-circle" src="uplode/<?php echo $uid.'.jpg';?>" alt="User profile picture">
							
							<?php $user_nm=$_SESSION['name'];
							$query="select * from multi_login WHERE `id`='$uid'";
