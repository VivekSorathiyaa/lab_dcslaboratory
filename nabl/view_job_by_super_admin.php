...
$up_job="update job set `report_received`=1,`job_owner`=2,`eng_light_status`=1 where `report_no`='$txt_report' AND `job_number`='$txt_jobs'";
mysqli_query($conn,$up_job);


//  update final_material_assign_master table if report not available in for this report no

$sel_final_material="select * from final_material_assign_master where `report_no`='$txt_report' AND `eng_light_status`!='2'";
$query_final_material=mysqli_query($conn,$sel_final_material);

if(!mysqli_num_rows($query_final_material) > 0){
	
	$up_job="update job set `eng_light_status`=2 where `report_no`='$txt_report' AND `job_number`='$txt_jobs'";
	mysqli_query($conn,$up_job);
}
?>
										...
										<table class="table no-margin">
										  ...
										  <?php
											...
											if($job_row["morr"]=="r"){
												...
												$up_final="update final_material_assign_master set `light_status`='2' where `report_no`='$_GET[report_no]'";
...
$(document).on("click", ".report_send_to_q_manager", function () {
	...
	$.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>save_span_engineer.php',
        data: 'action_type=send_to_qlty_manager&clicked_id='+clicked_id+'&txt_report_no='+txt_report_no,
        success:function(html){
			window.location.href="<?php echo $base_url; ?>view_job_by_eng.php?report_no="+txt_report_no+'&&job_no='+job_no;
