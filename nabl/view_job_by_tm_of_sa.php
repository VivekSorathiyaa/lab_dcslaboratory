                                    ...
                                    <div class="table-responsive" id="display_data">
                                        <table class="table table-bordered no-margin">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($array_labs_nos as $one_labs_nos) {
                                                    ...
                                                    while ($row = mysqli_fetch_array($result)) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <input disabled type="text" name="startdate" class="form-control startdate_class" id="startdate_<?php echo $material_count; ?>" value="<?php if ($is_est == 1) {
                                                            ...
                                                            <td>
                                                                <input disabled type="text" name="enddate" class="form-control enddate_class" id="enddate_<?php echo $material_count; ?>" value="<?php if ($is_est == 1) {
                                                            ...
                                                            <td>
																...
																<?php
                                                                    ...
                                                                    ?>
                                                                            ...
                                                                            <a href="<?php echo $base_url . "scan_document/" . $get_jobs['scan_document'] ?>" class="btn btn-success  btn3d" title="Downlaod Document" download><span class="glyphicon glyphicon-question-downlaod"></span> Download</a>

                                                                    <?php }
                                                                    } ?>
																<a href="upload_wriiten_obs/<?php echo $row['upload_obs'];?>" class="btn btn-primary" target="_blank" title="VIEW DOCUMENT">OBS</a>
																
																<input type="button" value="Notes" class="btn btn-info btn3d btn_witness" data-id="<?php echo $row['final_material_id'];?>" data-toggle="modal" data-target="#modal-witness" style="margin-top:20px;">
																
																		<?php if($row["accept_by_tm"]=="0"){ ?>
																		 <a href="javascript:void(0);" class="btn btn-success btn-lg btn3d accept_by_tm_of_qm" data-id="<?php echo $row['lab_no'].'|'.$row['trf_no'].'|'.$row['final_material_id']; ?>" title="Report Send To Quality Managetr"><span class="glyphicon glyphicon-question-list"></span> Accept</a>
																		<?php }
																		else{ echo '<span style="color:green;font-weight:bold;">Accepted</span>';}?>
                                                ...
                                                $select_job_for_eng = "select * from job_for_engineer where `accept_by_tm`=0 AND `trf_no` = '$get_trf_no'";
                                                $query_job_for_eng = mysqli_query($conn, $select_job_for_eng);
                                                if (mysqli_num_rows($query_job_for_eng) == 0) {
                                                    
													$upd_jobs="update job set `all_accept_by_tm`='1' where `trf_no`='$get_trf_no'";
              ...
              <div class="modal-header">
				...
				<form id="form_city" name="form_city" method="post">
					<div class="modal-body">
						...
						<div class="row">
							<div class="col-md-12">
								<label for="inputEmail3" class="col-sm-12 control-label">Note:</label>
								<input type="text" placeholder="Enter Note" id="txt_note" name="txt_note" class="form-control">
							</div>
						</div>
							
						</div>
						<input type="hidden" value="" id="txt_hidden_final_id">
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default pull-left" data-dismiss="modal">Close</button>
						<button type="button" class="btn btn-primary" id="update_witness" name="update_witness" data-dismiss="modal">Update Note</button>
					</div>
				</form>
            </div>
            <!-- /.modal-content -->
...
<script>
    ...
    $(document).on("click", ".accept_by_tm_of_qm", function() {
            ...
            buttons: {
                confirm: function() {
                    $.ajax({
                        type: 'POST',
                        url: '<?php $base_url; ?>save_span_engineer.php',
                        data: 'action_type=accept_by_tm_of_qm&clicked_id=' + clicked_id,
...
$(document).on("click", ".btn_witness", function() {
        var clicked_id = $(this).attr("data-id");
        $.ajax({
            type: 'POST',
			dataType: 'JSON',
            url: '<?php $base_url; ?>save_span_engineer.php',
            data: 'action_type=get_notes&clicked_id=' + clicked_id,
            success: function(data) {
				$("#txt_note").val(data.notes_by_tm);
				$("#txt_hidden_final_id").val(data.final_material_id);
				
			}
        });
    });
	
	$(document).on("click", "#update_witness", function() {
        var txt_note=$("#txt_note").val();
        var final_material_id=$("#txt_hidden_final_id").val();
        $.ajax({
            type: 'POST',
			dataType: 'JSON',
            url: '<?php $base_url; ?>save_span_engineer.php',
            data: 'action_type=update_notes&txt_note=' + txt_note+'&final_material_id='+final_material_id,
            success: function(data) {
				location.reload();
			}
        });
