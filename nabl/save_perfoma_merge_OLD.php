	...
	} else if($_POST['action_type'] == 'on_make_by_change')
	{
		...
		?>
			...
			array_push($now_array,$name_of_works);										  
											...
											foreach($explode_mat_ids as  $mt_keys => $one_mates)
											{
												
														// if test id not in array
														if (!in_array($explode_test_ids[$mt_keys], $merge_test_id_array))
														{
															array_push($merge_material_id_array,$one_mates);
															array_push($merge_material_name_array,$explode_mat_name[$mt_keys]);
															array_push($merge_test_id_array,$explode_test_ids[$mt_keys]);
															array_push($merge_test_name_array,$explode_test_name[$mt_keys]);
															array_push($merge_test_qty_array,$explode_test_qty[$mt_keys]);
															array_push($merge_test_rate_array,$explode_test_rates[$mt_keys]);
															array_push($merge_test_amt_array,$explode_test_rates[$mt_keys]);
														}
														else
														{
															// plus qty in qty_array
															$keys = array_search($explode_test_ids[$mt_keys], $merge_test_id_array);
															...
															$keying = array_search($explode_test_ids[$mt_keys], $merge_test_id_array);
															$get_amt_by_key=$merge_test_amt_array[$keying];
															$plus_amt= intval($get_amt_by_key)+ intval($ratingses);
	  ...
	  $get_ests=mysqli_query($conn,$sel_ests);
		if(mysqli_num_rows($get_ests) > 0)
		{
			...
			foreach($explode_mat_ids as  $mt_keys => $one_mates)
			{
				...
				if (!in_array($one_mates, $get_material_id_array))
				{	
					array_push($get_material_id_array,$one_mates);
					array_push($get_material_name_array,$explode_mat_name[$mt_keys]);
					array_push($get_material_qty_array,$explode_mat_qty[$mt_keys]);
					array_push($get_material_rate_array,$explode_mat_rates[$mt_keys]);
				}
				else{
					$keys = array_search ($one_mates, $get_material_id_array);
					$plus= intval($get_material_qty_array[$keys]) + intval($explode_mat_qty[$mt_keys]);
