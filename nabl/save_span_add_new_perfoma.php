...
if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
    if($_POST['action_type'] == 'get_data_by_rate'){
		...
		$txt_grand=$select_exploded[5];
				...
				if($gst_type=="with_gst" && $in_or_ex=="include"){
					...
					$count_gst= $txt_grand / 1.18;
					$both_gst_c_s= $txt_grand- $count_gst;
					$cgst= $both_gst_c_s/2;
					$sgst= $both_gst_c_s/2;
					$igst=0;
					
					$grand_total= $txt_grand- $both_gst_c_s;
				...
				if($gst_type=="with_gst" && $in_or_ex=="exclude"){
					...
					$count_gst= $txt_grand * 1.18;
					$both_gst_c_s= $count_gst- $txt_grand;
					$cgst= $both_gst_c_s/2;
					$sgst= $both_gst_c_s/2;
					$igst=0;
				...
				$fill=array("cgst" => number_format($cgst,2),"sgst" => number_format($sgst,2),"igst" => number_format($igst,2),"grand_total" => number_format($grand_total,2),"net_total" => $net_total,"get_total_amt_in_word" => $get_total_amt_in_word);
    ...
    }else if($_POST['action_type'] == 'add_estimate')
	{
		...
		$get_job_data = "select * from job WHERE `tfr_no`='$txt_trf_no'";
		...
		if(mysqli_num_rows($resultset)>0)
		{
			...
			if($report_sent_to=="0")
			{
				$update_gst = "update job set client_gstno='$gst_no' where `tfr_no`='$txt_trf_no'";				
			}
			else
			{
				$update_gst = "update job set agency_gstno='$gst_no' where `tfr_no`='$txt_trf_no'";
			}
	...
	}else if($_POST['action_type'] == 'save_next_estimate_only_save')
	{
		...
		$txt_discount_class_array= $_POST["txt_discount_class"];
		$txt_discount_percent= $_POST["txt_discount_percent"];
		$txt_discount_amount= $_POST["txt_discount_amount"];
		...
		$todays=date("Y-m-d");
		$insert_estimate="insert into quotations(`quatation_no`,`quatation_date`,`agency_id`,`gst_type`,`gst_in_or_ex`,`test_ids`,`test_name`,`test_qty`,`test_rates`,`test_totals`,`c_gst_amt`,`s_gst_amt`,`i_gst_amt`,`grand_total`,`total_amt`,`total_amt_in_word`,`est_creatredby`,`est_createddate`,`est_modifyby`,`est_modifydate`,`mate_name`,`mat_ids`,`name_of_work`,`discount_percent`,`txt_discount_percentage`,`discount_amount`)
						values(
						'$perfoma_no',
						'$invoice_date',
						'$hidden_agency',
						...
						'$name_of_work',
						'$txt_discount_percent',
						'$txt_discount_class_array',
						'$txt_discount_amount')";
						$result_insert_estimate=mysqli_query($conn,$insert_estimate);
	}else if($_POST['action_type'] == 'edit_qoutation')
	{
		...
		$test_mate_id_array= $_POST["test_mate_id_array"];
		$txt_discount_class_array= $_POST["txt_discount_class"];
		$quotations_id= $_POST["quotations_id"];
		$txt_discount_percent= $_POST["txt_discount_percent"];
		$txt_discount_amount= $_POST["txt_discount_amount"];
		
		
		$up_qua="update quotations set `quatation_no`='$perfoma_no',`quatation_date`='$invoice_date',`agency_id`='$hidden_agency',`gst_type`='$hidden_gst_type',`gst_in_or_ex`='$hidden_gst_in_ex',`test_ids`='$test_ids_array',`test_name`='$test_name_array',`test_qty`='$test_qty_array',`test_rates`='$test_rates_array',`test_totals`='$test_amnt_array',`c_gst_amt`='$txt_cgst',`s_gst_amt`='$txt_sgst',`i_gst_amt`='$txt_igst',`grand_total`='$txt_grand',`total_amt`='$total_amt',`total_amt_in_word`='$txt_amt_in_word',`mate_name`='$test_mate_array',`mat_ids`='$test_mate_id_array',`name_of_work`='$name_of_work',`discount_percent`='$txt_discount_percent',`discount_amount`='$txt_discount_amount',`txt_discount_percentage`='$txt_discount_class_array' where `quotations_id`=".$quotations_id;
		
		$result_insert_estimate=mysqli_query($conn,$up_qua);
	}
	
	else if($_POST['action_type'] == 'delete_quatation')
	{
		$id=$_POST["id"];
		$dels="delete from quotations where `quotations_id`=".$id;
		mysqli_query($conn,$dels);
		$fill = array("done");
					    echo json_encode($fill);
		
	}else if($_POST['action_type'] == 'change_status')
	{
		$id=explode("|",$_POST["id"]);
		$q_id=$id[0];
		$q_status=$id[1];
		if($q_status=="pending"){ $sets="complete";} 
		if($q_status=="complete"){ $sets="cancel";}
		if($q_status=="cancel"){ $sets="pending";}
		
		$dels="update quotations set `statues`='$sets' where `quotations_id`=".$q_id;
		mysqli_query($conn,$dels);
		$fill = array("done");
					    echo json_encode($fill);
		
	}else if($_POST['action_type'] == 'direct_perfoma_excel_upload')
	{
		...
		if(isset($_FILES["file"]["name"]))
			{
				...
				$filename= "direct_perfoma_".rand(999,99999).".".$ext;
	...
	else if($_POST['action_type'] == 'direct_perfoma_excel_upload_update')
	{
		...
		if(isset($_FILES["file"]["name"]))
			{
				...
				$path = 'uploaded_excel/';
				move_uploaded_file($tmp_name,$path.$filename);
				$fileings=", `perfoma_excel_upload`='$filename'";
			}else{
				$filename="";
				$fileings="";
				...
				$update_perfoma="update estimate_total_span set `gst_no`='$gst_no',`hsn_codes`='$hsn_codes',`estimate_date`='$invoice_date',`agency_id`='$hidden_agency',`rate_type`='$select_rate',`gst_type`='$hidden_gst_type',`gst_in_or_ex`='$hidden_gst_in_ex',`c_gst_amt`='$txt_cgst',`s_gst_amt`='$txt_sgst',`i_gst_amt`='$txt_igst',`grand_total`='$txt_grand',`total_amt`='$total_amt',`total_amt_in_word`='$txt_amt_in_word',`customer_name`='$customer_name',`name_of_work`='$name_of_work',`agreement_no`='$agreement_no',`reference_no`='$reference_no'".$fileings." where `perfoma_no`='$perfoma_no'";
				
				$result_update_perfoma=mysqli_query($conn,$update_perfoma);
				$fill = array($result_update_perfoma);
			    echo json_encode($fill);
	}
	else if($_POST['action_type'] == 'update_new_direct_perfoma')
	{
		...
		$update_perfomas="update estimate_total_span set `gst_no`='$gst_no',`hsn_codes`='$hsn_codes',`rate_type`='$select_rate',`gst_type`='$hidden_gst_type',`gst_in_or_ex`='$hidden_gst_in_ex',`test_ids`='$test_ids_array',`test_name`='$test_name_array',`test_qty`='$test_qty_array',`test_rates`='$test_rates_array',`test_totals`='$test_amnt_array',`c_gst_amt`='$txt_cgst',`s_gst_amt`='$txt_sgst',`i_gst_amt`='$txt_igst',`grand_total`='$txt_grand',`total_amt`='$total_amt',`total_amt_in_word`='$txt_amt_in_word',`mate_name`='$test_mate_array',`mat_ids`='$test_mate_id_array',`agency_id`='$hidden_agency',`customer_name`='$customer_name',`agreement_no`='$agreement_no',`reference_no`='$reference_no',`name_of_work`='$name_of_work' where `perfoma_no`='$perfoma_no'";
		
		$result_perfoma=mysqli_query($conn,$update_perfomas);
	}
	else if($_POST['action_type'] == 'set_perfoma_no_by_invoice_date')
	{
			   ...
			   if (mysqli_num_rows($select_result_test) > 0) 
			{
				...
				foreach($exploded_test_array as $one_test) 
				{
				   $get_query_test_names = "select test_name from test_master WHERE `test_isdeleted`='0' AND `test_id`=".$one_test; 
					...
					if($get_test_name['test_name']!=""){
					$out_tests .='<option value="'.$one_test."|".$get_test_name['test_name'].'" >'.$get_test_name['test_name'].'</option>';
					}
				}	
			}   	
	 ...
	 $fill = array('all_material' => $out_materials,'get_rows_counts' => $get_rows_counts,'category_names' => $category_names);	
        echo json_encode($fill);
    }elseif($_POST['action_type'] == 'get_rates_by_test_id'){
		...
		$sel_test_rate="select * from test_master where `mat_category_id`='$mat_cat_id' AND `test_id`='$select_test'";
		$result_insert_test_wise=mysqli_query($conn,$sel_test_rate);
		
		if(mysqli_num_rows($result_insert_test_wise) > 0)
		{
			$row = mysqli_fetch_array($result_insert_test_wise);
			$testings=$row["test_rate_private"];
		}else{
			$testings=0;
		}
		   	
		
		$fill = array('msg' => "Done",'testings' => $testings);	
		echo json_encode($fill);
    }
}
