   ...
   $select_mate_counts="select `final_material_id`,`material_category` from final_material_assign_master WHERE `is_deleted`=0 AND `temporary_trf_no`='$txt_final_trf_no' AND `created_by_id`='$_SESSION[u_id]' ORDER BY final_material_id ASC";
	...
	$get_counts_of_materials=mysqli_num_rows($result_mate_counts);
		...
		if($get_counts_of_materials > 0)
		{
			...
			$explode_rec_date=explode("-",$sample_rec_date);
			$job_no_date=$explode_rec_date[0].$explode_rec_date[1].$explode_rec_date[2];
			$months="_____".$explode_rec_date[1]."___";
			$nabl_type=$get_jobs_result["nabl_type"];
			$only_year=$explode_rec_date[0];
			$only_month=$explode_rec_date[1];
			$branch_id=$get_jobs_result["branch_id"];
			
			$sel_fy="select * from fyearmaster where `fy_isdeleted`=0";
			$query_fy=mysqli_query($conn,$sel_fy);
			$get_jobs_last=mysqli_fetch_array($query_fy);
			$fy_name=$get_jobs_last["fy_name"];
			...
			for($i=1;$i<=$get_counts_of_materials;$i++)
			{
				if($nabl_type=="nabl")
				{
							$sel_reserve_ulr="select * from ulr_sequence WHERE `ulr_sequence_date`='$sample_rec_date' AND `ulr_status`='3' ORDER BY ulr_sequence_id ASC LIMIT 0,1";
							$query_reserve_ulr=mysqli_query($conn,$sel_reserve_ulr);
							if(mysqli_num_rows($query_reserve_ulr) > 0)
							{
								$result_reserve_ulr=mysqli_fetch_assoc($query_reserve_ulr);
								$ulr_no= $result_reserve_ulr["ulr_sequence"];
								$set_ulr_no= sprintf('%09d', $ulr_no);
								$set_trf_no="DCS/".$only_year."/".$only_month."/".sprintf('%02d', $result_reserve_ulr["trf_no"]);
								$set_job_no=$result_reserve_ulr["job_no"];
								$set_report_no=$result_reserve_ulr["report_no"];
								$lab_no=$result_reserve_ulr["lab_no"];
								$ulr_type="updates";
								
								// update ulr_sequence table
								$update_seq="update ulr_sequence set `table_primary_key_id`='$final_id_array[$counting]',`ulr_status`='2' where `ulr_sequence`=".$ulr_no;
								mysqli_query($conn,$update_seq);
								
							}else{
								
								$ulr_type="inserted";
								// ulr seq
								$sel_last_of_ulr="select * from ulr_sequence ORDER BY ulr_sequence_id DESC LIMIT 0,1";
								...
								$ulr_no= $ulr_counts;
								$set_ulr_no= sprintf('%09d', $ulr_no);
								$set_trf_no="DCS/".$only_year."/".$only_month."/".sprintf('%02d', $trf_no);
								$set_job_no=$job_no_date."DCS".sprintf('%02d', $ulr_no);
								$set_report_no="DCS/".$explode_rec_date[1]."/".$_SESSION['fy_name']."/".sprintf('%02d', $ulr_no);
								...
								//if not in ulr table insert in this table
								$todays=date("Y-m-d");
								$user_ids=$_SESSION["u_id"];
								$ins_ulr_no="insert into ulr_sequence(`ulr_sequence`,`ulr_sequence_date`,`table_primary_key_id`,`ulr_status`,`created_date`,`created_by`,`modified_date`,`modified_by`,`branch_name`,`branch_short_code`,`trf_no`,`job_no`,`report_no`,`lab_no`,`trf_max_number`)VALUES($ulr_no,'$sample_rec_date','$final_id_array[$counting]','2','$todays','$user_ids','$todays','$user_ids','KANGRA','RJT','$trf_no','$set_job_no','$set_report_no','$lab_no',$trf_max_number)";
							...
							$insert_save_mate="insert into save_material_assign (`lab_no_counts`,`temporary_trf_no`,`day_seq`,`max_number`,`trf_no`, `job_no`, `isstatus`, `created_by`, `created_date`, `modified_by`, `modified_date`,`nabl_type`,`rec_sam_date`,`branch_id`,`branch_name`,`branch_short_code`) 
										values(
										'$lab_no',
										'$txt_final_trf_no',
										'0',
							...
							$maxs=0;
							$print_fs=
							$txt_set_lab_nos= $get_cat_prefix."-".sprintf('%02d', $lab_no)."/".$explode_rec_date[1]."/".$_SESSION['fy_name']."/".sprintf('%02d', $ulr_no);
							
							$update_ulrs="update final_material_assign_master set `ulr_no`='$set_ulr_no' where `final_material_id`=".$final_id_array[$counting];
				...
				$update_final_mate="update final_material_assign_master set `trf_no`='$set_trf_no',`job_no`='$set_job_no',`lab_no`='$txt_set_lab_nos',`report_no`='$set_report_no',`max_number`='$maxs',`is_status`=1 where `final_material_id`=".$final_id_array[$counting];
				...
				<div class="col-md-12">
					<div class="box box-info">
							...
							<div class="panel-group">
								...
								<a data-toggle="collapse" href="#collapse1" class="btn btn-light-primary" style="width:100%;margin-top: 2%;font-size: 20px;" id="add_material_button">Add Material</a>
								...
								<div class="row">
								  ...
								  <div class="col-md-4">
									...
									<div class="col-md-4">
										<div class="form-group">
											<div class="col-sm-12">
												<input type="text"  class="form-control" name="ex_date_submission" id="ex_date_submission" value="<?php echo date('d/m/Y')?>">
											</div>
										</div>
									</div>
								...
								<div class="row">
									...
									<div class="col-md-4">
									<label for="exampleInputEmail1">No. Of Reports:</label><br>
								...
								<div class="row">
									...
									<div class="form-group">
										<div class="col-sm-12">
										<input type="text" class="" id="txt_qty" name="txt_qty" placeholder="Enter Quantity" value="1" disabled>
									    </div>
									</div>
									</div>
					...
					<div class="box box-info">
						<form class="form" id="billing" method="post">
							<div class="panel-group">
								<div class="panel panel-default">
									...
									<div class="panel-heading" style="background-color: unset;">
										<h4 class="panel-title" style="text-align:center;">
											<a data-toggle="collapse" href="#collapse2" class="btn btn-light-primary" style="width:100%;" aria-expanded="true"><b>Type Of Sample</b></a>
										</h4>
									</div>
									<div id="collapse2" class="panel-collaps" aria-expanded="true">
								...
								<div class="col-md-2">
								<input type="hidden" name="radio" id="radio" value="r">
...
$("#txt_qty").change(function(){
   ...
   // only for steel materials change 
	...
	var select_material_category = $('#select_material_category').val(); 
	  ...
	  if(select_material_category=="10" && steel_set_qty > 5){
		  alert("Please Enter Max 5 Quantity");
		  $('#steel_set_qty').val("0");
		  return false;
	  }
...
//save in final
	...
	$(document).on("click","#btn_add_data_save",function(){
				...
				var steel_set_qtyes=$("#steel_set_qty").val();
				// condition for steel_brand
				if(select_material_category !="" && select_material_category =="10" && dia=="")
				{
					//alert("Please Enter Dia First...");
				...
				var steeling_idss= $("#steeling_id").val();
				if(select_material_category =="10" && select_material =="135" && steeling_idss =="")
				{
				...
				/*if(select_material =="135" && select_material_category =="10" && steel_brand.length != txt_qty)
				{
					alert("Please Enter S First...");
					return false;
				}*/
				...
				if(select_material_category =="6" && select_material =="130" && color.length != txt_qty)
				{
				...
				if(select_material_category =="6" && select_material =="130" && thickness.length != txt_qty)
				{
					
					alert("Please Enter Data Properly");
					return false;
					
				}if(select_material_category =="7" && select_material =="131" && cement_brand.length != txt_qty)
				{
				...
				if(select_material_category =="10" && select_material =="212" && steeling_idss =="")
				{
...
$(document).on('change','.type_of_cement', function(event){
	var type_of_cements= $(this).val();
	var id_of_cement= $(this).attr("id");
	var explodings=id_of_cement.split("_");
	var set_ids="#c_grade_"+explodings[2];
	var designs="";
	if(type_of_cements == "OPC"){
		designs +='<option value="53 OPC">53 OPC</option>';
		designs += '<option value="43 OPC">43 OPC</option>';
		designs += '<option value="33 OPC">33 OPC</option>';
		
	}else if(type_of_cements == "PPC"){
		designs += '<option value="flyash_type">Flyash Type</option>';
		designs += '<option value="calcined_clay_type">Calcined Clay Type</option>';
		
	}else if(type_of_cements == "PSC"){
		designs += '<option value="33_grade">33 GRADE</option>';
		
	}else{
		designs = '<option value="">Select Grade</option>';
	}
	$(set_ids).html(designs);
	
});

$(document).on('change','.ingrade', function(event){
	var grd= $(this).val();
	var id_of_cement= $(this).attr("id");
	var explodings=id_of_cement.split("_");
	var set_ids="#inden_"+explodings[1];
