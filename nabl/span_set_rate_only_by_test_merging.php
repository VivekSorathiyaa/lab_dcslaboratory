...
$select_test_wise="select * from test_wise_material_rate where `trf_no`='$txt_trf_no' AND `job_no`='$txt_jobs'";
 ...
 $sel_estiamte="select * from estimate_total_span_only_for_test where `trf_no`='$_GET[chk_array]' AND `job_no`='$_GET[chk_array]'  AND `est_isdeleted`=0";
...
$("#select_rate").change(function(){
      ...
      var postData = 'action_type=get_data_by_rate&select_rate='+select_rate+'&test_id_array='+test_id_array+'&test_qty_array='+test_qty_array;
		  ...
		  return false;
			...
			$.ajax({
				url : "<?php $base_url; ?>save_span_set_rate_for_test_merging.php",
				type: "POST",
				data : postData,
				dataType:'JSON',
		...
		if(hidden_gst_type !="without_gst")
		{
			...
			else
			{
				...
				success:function(msg){
					...
					window.location.href="<?php echo $base_url; ?>span_set_rate_only_by_test_merging.php?chk_array="+txt_trf_no;
