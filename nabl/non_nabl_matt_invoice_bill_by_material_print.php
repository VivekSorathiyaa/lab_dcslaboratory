...
<?php
		...
		$explode_temporary_trf_no_array=explode(",",$temporary_trf_no_array);

		$txt_temporary_trf_no= $explode_temporary_trf_no_array[0];

		$sel_estiamte="select * from estimate_total_span_only_for_material where `trf_no`='$chk_array' AND `job_no`='$chk_array' AND `nabl_type`='non_nabl' AND `temporary_trf_no`='$temporary_trf_no_array'";
		...
		$refno="";
	  foreach($explode_chk_array as $keyings => $one_trf_nos)
	  {
		  $sel_job="select * from job where `trf_no`='$one_trf_nos' AND `temporary_trf_no`='$explode_temporary_trf_no_array[$keyings]'";
		  $result_job =mysqli_query($conn_of_non,$sel_job);
		  $row_job =mysqli_fetch_array($result_job);
		  $refno .= '<span style="min-width:100px;">'.$row_job["refno"].'</span>'."&nbsp;&nbsp;&nbsp;&nbsp;Date:-&nbsp;&nbsp;".date('d-m-Y',strtotime($row_job["date"]))."<br>";

		  $update_jobs= "update job set `print_done_by_biller_for_qm_see`=1 where `trf_no`='$one_trf_nos'  AND `temporary_trf_no`='$explode_temporary_trf_no_array[$keyings]'";
