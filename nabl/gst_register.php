...
<section class="content" style="padding: 0px;
     ...
     padding-right: 0px; ">
			...
			<div class="col-md-3">
			<h1 style="text-align:center;">GST REGISTER</h1>
			</div>
			</div>
			...
			<!--<div class="col-md-2">
								<label>Other:</label>
									<select class="form-control select2" name="sel_other" id="sel_other" >
									<option value="">Select-Other</option>
									<?php
									//$clients="select * from estimate_total_span where `bill_to_name`!=''";
									//$query_client=mysqli_query($conn,$clients);
									//if(mysqli_num_rows($query_client) > 0)
									//{
									//	while($one_client=mysqli_fetch_array($query_client))
									//	{
									?>
										<option value="<?php //echo $one_client["bill_to_name"];?>"><?php //echo $one_client["bill_to_name"];?></option>
									<?php
									//	}
									//}
									?>
...
function search_data(type,id){
    ...
    if (type == 'search_data') {
				...
				billData = '&action_type='+type+'&user_type='+user_type+'&from_date='+from_date+'&to_date='+to_date;		
    }
	
    $.ajax({
        type: 'POST',
        url: '<?php $base_url; ?>search_gst_register.php',
        data: billData,
		beforeSend: function(){
